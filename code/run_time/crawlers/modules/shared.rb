...
module Shared
  ...
  def select_max(id, value)
    ...
    begin
      ...
      @logger.warn("#{__FILE__} => #{method_name}") {"#{@kte.company} => Indemnity limit not matched! Using max available value with selected label #{page.get_selected_label(@last_element)}"}
    end
  end
