...
class LinearSect2 < Test::Unit::TestCase
  ...
  def type_text(id, value = nil)
   ...
   def click_option(id, value = nil)
    @last_element, @last_value = id, value
    @logger.debug("#{__FILE__} => #{method_name}") {"#{@kte.company} => now's checked option button element: [#{@last_element}] with value [#{@last_value}]"}
