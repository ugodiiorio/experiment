module FieldMappingProvider1Sect1
  ...
  def build_hash_field_rule()
    ...
    @field_rule_values = {
      ...
      'pol_first_claim_year_str' => ' if claimslastyr = infield[:pol_nr_of_paid_claims_this_yr_str].to_i == 1 ; (DateTime::now).strftime("%G") elsif claimslastyr = infield[:pol_nr_of_paid_claims_1_yr_str].to_i == 1 ; (DateTime::now - 1.year).strftime("%G"); elsif claimslastyr = infield[:pol_nr_of_paid_claims_2_yr_str].to_i == 1 ; (DateTime::now - 2.year).strftime("%G"); elsif claimslastyr = infield[:pol_nr_of_paid_claims_3_yr_str].to_i == 1 ; (DateTime::now - 3.year).strftime("%G"); end ;' ,
      'pol_second_claim_month_str' => '"Gennaio"',
      'pol_second_claim_year_str' => 'claimscert = infield[:pol_claims_total_number_str]; claimslastyr = infield[:pol_nr_of_paid_claims_this_yr_str]; if claimscert.to_i == 2 && claimslastyr.to_i == 1 ; (DateTime::now - 2.year).strftime("%G") elsif claimscert.to_i == 2 && claimslastyr.to_i == 0 ; (DateTime::now - 3.year).strftime("%G") else (DateTime::now - 1.year).strftime("%G"); end; ',
