module FieldMappingProvider1Sect1
  ...
  def build_hash_field_rule()
    ...
    @field_rule_values = {
      ...
      'driv_driving_license_year_of_issue_str' =>  '(DateTime::now - infield[:driv_driver_sex_str].slice(1, 2).to_i.years-2.months + 18.years).strftime("%Y")',
      ...
      'pol_birth_date_day_str' =>  'infield[:driv_driver_sex_str].strip.length == 3 ? (DateTime::now - infield[:driv_driver_sex_str].slice(1 ,2).to_i.years-2.months).strftime("%F").slice(8,2) : (DateTime::now - 30.years-2.months).strftime("%F").slice(8,2)',
      'pol_birth_date_month_str' =>  'infield[:driv_driver_sex_str].strip.length == 3 ? (DateTime::now - infield[:driv_driver_sex_str].slice(1 ,2).to_i.years-2.months).strftime("%F").slice(5,2) : (DateTime::now - 30.years-2.months).strftime("%F").slice(5,2)',
      'pol_birth_date_str' =>  'infield[:driv_driver_sex_str].strip.length == 3 ? x=(DateTime::now - infield[:driv_driver_sex_str].slice(1, 2).to_i.years-2.months).strftime("%F") : x=(DateTime::now - 30.years-2.months).strftime("%F")',
      'pol_birth_date_year_num' =>  ' infield[:driv_driver_sex_str].strip.length == 3 ? (DateTime::now - infield[:driv_driver_sex_str].slice(1 ,2).to_i.years-2.months).strftime("%F").slice(0,4).to_i : (DateTime::now - 30.years-2.months).strftime("%F").slice(0,4).to_i',
      ...
      'pol_second_claim_year_str' => 'claimscert = infield[:pol_claims_total_number_str]; claimslastyr = infield[:pol_nr_of_paid_claims_this_yr_str]; if claimscert.to_i == 2 && claimslastyr.to_i == 1 ; (DateTime::now - 2.year).strftime("%Y") elsif claimscert.to_i == 2 && claimslastyr.to_i == 0 ; (DateTime::now - 3.year).strftime("%Y") else (DateTime::now - 1.year).strftime("%Y"); end; ',
      ...
      'veh_make_str' =>  'a=infield[:veh_make_str].strip; b= a.gsub(/à/,"a" );c= b.gsub(/è/,"e" );d= c.gsub(/é/,"e" );e= d.gsub(/ì/,"i" );f= e.gsub(/ò/,"o" );g= f.gsub(/ù/,"u" ); h= g.gsub(/°/,"a" );',
      'veh_model_str' =>  'a=infield[:veh_model_str].strip; b= a.gsub(/à/,"a" );c= b.gsub(/è/,"e" );d= c.gsub(/é/,"e" );e= d.gsub(/ì/,"i" );f= e.gsub(/ò/,"o" );g= f.gsub(/ù/,"u" ); h= g.gsub(/°/,"a" );',
