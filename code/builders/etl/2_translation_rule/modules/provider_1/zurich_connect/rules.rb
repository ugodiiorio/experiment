...
module Provider1ZurichConnect
  ...
  def build_hash_sect_1()
    ...
    @rule_values = {
      ...
      'pol_residence_str' => 'res=profilefield[:pol_residence_str].gsub("SAN GIOVANNI IN PERSICETO","SAN GIOVANNI IN PERSICET").gsub("TREVISAGO","TERVISAGO").gsub("DOLCE","DOLCE\'").gsub("PORTO SANT ELPIDIO","PORTO SANT\'ELPIDIO").gsub("REGGIO NELL\'EMILIA","REGGIO EMILIA").gsub("FOSSO","FOSSO\'").gsub("SANT ANNA ARRESI","SANT\'ANNA ARRESI").gsub("ROMANO D EZZELINO","ROMANO D\'EZZELINO").gsub("SEZZE SCALO","SEZZE").gsub("GUIDONIA MONTECELIO","GUIDONIA").gsub("TOSCOLANO MADERNO","TOSCOLANO").gsub("BELLARIA IGEA MARINA","BELLARIA").gsub("ROVERE VERONESE","ROVERE\' VERONESE").gsub("VILLA D ALME", "VILLA D\'ALME\'").gsub("TORRE DE PASSERI", "TORRE DE\' PASSERI").gsub("PONTE SAN NICOLO","PONTE SAN NICOLO\'").gsub("ABANO TERME","ABANO TERME BAGNI").gsub("RIVE D ARCANO","RIVE D\'ARCANO").gsub("CASTIGLIONE STIVIERE","CASTIGLIONE DELLE STIVIE").gsub("MONFORTE D ALBA","MONFORTE D\'ALBA").gsub("ARZAGO D ADDA","ARZAGO D\'ADDA").gsub("MONTEGROSSO D ASTI","MONTEGROSSO D\'ASTI").gsub("MUGGIO","MUGGIO\'").gsub("SANREMO","SAN REMO").gsub("CASTELNUOVO MAGRA","MOLICCIARA")',
