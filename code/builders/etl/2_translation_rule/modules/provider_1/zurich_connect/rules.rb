...
module Provider1ZurichConnect
  ...
  def build_hash_sect_1()
    ...
    @rule_values = {
      ...
      'pol_residence_str' => 'res=profilefield[:pol_residence_str].gsub("SAN GIOVANNI IN PERSICETO","SAN GIOVANNI IN PERSICET").gsub("TREVISAGO","TERVISAGO").gsub("DOLCE","DOLCE\'").gsub("SANT ELPIDIO","SANT\'ELPIDIO").gsub("REGGIO NELL\'EMILIA","REGGIO EMILIA").gsub("FOSSO","FOSSO\'").gsub("SANT ANNA","SANT\'ANNA").gsub("D EZZELINO","D\'EZZELINO").gsub("SEZZE SCALO","SEZZE").gsub("GUIDONIA MONTECELIO","GUIDONIA").gsub("TOSCOLANO MADERNO","TOSCOLANO").gsub("BELLARIA IGEA MARINA","BELLARIA").gsub("ROVERE VERONESE","ROVERE\' VERONESE").gsub("D ALME", "D\'ALME\'").gsub("DE PASSERI", "DE\' PASSERI").gsub("SAN NICOLO","SAN NICOLO\'").gsub("ABANO TERME","ABANO TERME BAGNI").gsub("D ARCANO","D\'ARCANO").gsub("CASTIGLIONE STIVIERE","CASTIGLIONE DELLE STIVIE").gsub("D ALBA","D\'ALBA").gsub("D ADDA","D\'ADDA").gsub("D ASTI","D\'ASTI").gsub("MUGGIO","MUGGIO\'").gsub("SANREMO","SAN REMO").gsub("CASTELNUOVO MAGRA","MOLICCIARA").gsub("SAN GIULIANO MILANESE","S.GIULIANO MILANESE").gsub("SANT ELENA","SANT\'ELENA").gsub("L AQUILA","L\'AQUILA")',
