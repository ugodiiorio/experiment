...
module Provider1Genertel
  ...
  def build_hash_sect_1()
    ...
    @rule_values = {
      ...
      'driv_birth_place_str' => 'res=profilefield[:driv_birth_place_str];res=res.gsub("CHIARAVALLE","CHIARAVALLE|CHIARAVALLE (AN)").gsub("CASTELNUOVO DEL GARDA","CASTELNUOVO|CASTELNUOVO (VR)").gsub("CASTEL SAN GIOVANNI","CASTEL SAN GIOVANNI|CASTEL SAN GIOVANNI (PC)").gsub("PRATO","PRATO|PRATO (PO)").gsub("UGGIATE TREVANO","UGGIATE-TREVANO").gsub("TOSCOLANO MADERNO","TOSCOLANO-MADERNO").gsub("BELLARIA IGEA MARINA","BELLARIA-IGEA MARINA").gsub("ROVERE VERONESE","ROVERE\' VERONESE").gsub("FOSSO","FOSSO\'").gsub("SANT ANNA","SANT\' ANNA").gsub("D EZZELINO","D\'EZZELINO").gsub("SEZZE SCALO","SEZZE").gsub("REGGIO NELL\'EMILIA","REGGIO EMILIA").gsub("VILLA D ALME", "VILLA D\'ALME").gsub("DE PASSERI", "DE\' PASSERI").gsub("SANT ELPIDIO","SANT\'ELPIDIO").gsub("S CASCIANO", "SAN CASCIANO").gsub("DOLCE","DOLCE\'").gsub("D ARCANO","D\'ARCANO").gsub("CASTIGLIONE STIVIERE","CASTIGLIONE DELLE STIVIERE").gsub("BRISSAGO VALTRAVAGLIA","BRISSAGO-VALTRAVAGLIA").gsub("D ALBA","D\'ALBA").gsub("D ADDA","D\'ADDA").gsub("D ASTI","D\'ASTI").gsub("COCQUIO TREVISAGO","COCQUIO-TREVISAGO").gsub("CESANA BRIANZA","CESANA").gsub("SANT ELENA","SANT\'ELENA").gsub("L AQUILA","L\'AQUILA").gsub(" SAGITTARIA","").gsub("D ESI","D\'ESI").gsub("SANREMO","SAN REMO")',
