...
#      'driv_birth_place_str' => 'res=profilefield[:driv_birth_place_str].gsub("CHIARAVALLE","CHIARAVALLE|CHIARAVALLE (AN)").gsub("CASTELNUOVO DEL GARDA","CASTELNUOVO|CASTELNUOVO (VR)").gsub("CASTEL SAN GIOVANNI","CASTEL SAN GIOVANNI|CASTEL SAN GIOVANNI (PC)").gsub("PRATO","PRATO|PRATO (PO)").gsub("UGGIATE TREVANO","UGGIATE-TREVANO").gsub("TOSCOLANO MADERNO","TOSCOLANO-MADERNO").gsub("BELLARIA IGEA MARINA","BELLARIA-IGEA MARINA").gsub("ROVERE VERONESE","ROVERE\' VERONESE").gsub("FOSSO","FOSSO\'").gsub("SANT ANNA","SANT\' ANNA").gsub("D EZZELINO","D\'EZZELINO").gsub("SEZZE SCALO","SEZZE").gsub("REGGIO NELL\'EMILIA","REGGIO EMILIA").gsub("VILLA D ALME", "VILLA D\'ALME").gsub("DE PASSERI", "DE\' PASSERI").gsub("SANT ELPIDIO","SANT\'ELPIDIO").gsub("S CASCIANO", "SAN CASCIANO").gsub("DOLCE","DOLCE\'").gsub("D ARCANO","D\'ARCANO").gsub("CASTIGLIONE STIVIERE","CASTIGLIONE DELLE STIVIERE").gsub("BRISSAGO VALTRAVAGLIA","BRISSAGO-VALTRAVAGLIA").gsub("D ALBA","D\'ALBA").gsub("D ADDA","D\'ADDA").gsub("D ASTI","D\'ASTI").gsub("COCQUIO TREVISAGO","COCQUIO-TREVISAGO").gsub("CESANA BRIANZA","CESANA").gsub("SANT ELENA","SANT\'ELENA").gsub("L AQUILA","L\'AQUILA").gsub(" SAGITTARIA","").gsub("D ESI","D\'ESI").gsub("SANREMO","SAN REMO").gsub(" FABIASCO","-FABIASCO").gsub("DELL EMILIA","DELL\'EMILIA").gsub("FORLI","FORLI\'").gsub(" AURISINA","").gsub(" DELLE MOLE","").gsub("CITTA ","CITTA\' ").gsub("SULL ARNO","SULL\'ARNO").gsub("IESOLO","JESOLO").gsub("SANT ANGELO LODIGIANO","SANT\'ANGELO")',
#      'driv_birth_place_str' => 'copy_field',
      ...
      'own_owner_zip_code_str' => 'profilefield[:own_owner_zip_code_str].sub("47521","47023")',
      'own_owner_residence_province_str' =>  'translate_field'   ,
      'own_owner_residence_str' =>  'res=profilefield[:own_owner_residence_str].gsub("SCANZANO JONICO","SCANZANO JONICO|SCANZANO (MT)").gsub("SANT ","SANT\'").gsub("D A","D\'A").gsub("SULL ","SULL\'").gsub("SOMMARIVA DEL","SOMMARIVA").gsub("MONASTIER DI TREVISO","MONASTIER").gsub("CASTROCARO TERRA SOLE","CASTROCARO TERME E TERRA DEL SOLE").gsub("CUGLIATE FABIASCO","CUGLIATE-FABIASCO").gsub("CASTELL ","CASTELL\'").gsub("ROVERE VERONESE","ROVERE\' VERONESE").gsub("FOSSO","FOSSO\'").gsub("SANT ANNA","SANT\' ANNA").gsub("D EZZELINO","D\'EZZELINO").gsub("SEZZE SCALO","SEZZE").gsub("REGGIO NELL\'EMILIA","REGGIO EMILIA").gsub("VILLA D\'ALME", "VILLA D\'ALME\'").gsub("DE PASSERI", "DE\' PASSERI").gsub("SANT ELPIDIO","SANT\'ELPIDIO").gsub("S CASCIANO", "SAN CASCIANO").gsub("DOLCE","DOLCE\'").gsub("D ARCANO","D\'ARCANO").gsub("CASTIGLIONE STIVIERE","CASTIGLIONE DELLE STIVIERE").gsub("D ALBA","D\'ALBA").gsub("D ADDA","D\'ADDA").gsub("D ASTI","D\'ASTI").gsub("SANT ELENA","SANT\'ELENA").gsub("L AQUILA","L\'AQUILA").gsub("D ESI","D\'ESI").gsub(" FABIASCO","-FABIASCO").gsub("DELL EMILIA","DELL\'EMILIA").gsub("FORLI","FORLI\'").gsub(" AURISINA","").gsub("CITTA ","CITTA\' ").gsub("SULL ARNO","SULL\'ARNO").gsub("IESOLO","JESOLO").gsub("SANT ANGELO","SANT\'ANGELO")',
...
#      'pol_property_type_to_be_insured_str' => 'copy_field',
      ...
      'pol_residence_province_str' => 'if profilefield[:pol_residence_str] =~ /(BELLUSCO)|(meda)|(lissone)|(monza)|(varedo)|(limbiate)|(arcore)|(cesano maderno)|(ceriano laghetto)|(besana in brianza)|(nova milanese)|(desio)|(barlassina)/i ; "MONZA BRIANZA" ; elsif profilefield[:pol_residence_str] =~ /(MONTE URANO)|(PORTO SANT ELPIDIO)|(montegranaro)/i ; "FERMO" ; elsif profilefield[:pol_residence_str] =~ /(ANDRIA)/i ; "BARLETTA-ANDRIA-TRANI" ;else translate_field ; end',
      'pol_residence_str' => 'res=profilefield[:pol_residence_str].gsub("MONTECATINI TERME","NIEVOLE").gsub("SCANZANO JONICO","SCANZANO JONICO|SCANZANO (MT)").gsub("D A","D\'A").gsub("SULL ","SULL\'").gsub("SANT ","SANT\'").gsub("CASTELL ","CASTELL\'").gsub("SOMMARIVA DEL","SOMMARIVA").gsub("CAERANO DI","CAERANO").gsub("MONASTIER DI TREVISO","MONASTIER").gsub("MASERA","MASERA\'").gsub("VILLANOVA CAMPOSAMPIERO","VILLANOVA DI CAMPOSAMPIERO").gsub("MONTE COMPATRI","MONTECOMPATRI").gsub("SCORZE","scorze\'").gsub("CASTROCARO TERRA SOLE","CASTROCARO TERME E TERRA DEL SOLE").gsub("ROVERE VERONESE","ROVERE\' VERONESE").gsub("FOSSO","FOSSO\'").gsub("SANT ANNA","SANT\' ANNA").gsub("D EZZELINO","D\'EZZELINO").gsub("SEZZE SCALO","SEZZE").gsub("REGGIO NELL\'EMILIA","REGGIO EMILIA").gsub("VILLA D\'ALME", "VILLA D\'ALME\'").gsub("DE PASSERI", "DE\' PASSERI").gsub("SANT ELPIDIO","SANT\'ELPIDIO").gsub("S CASCIANO", "SAN CASCIANO").gsub("DOLCE","DOLCE\'").gsub("D ARCANO","D\'ARCANO").gsub("CASTIGLIONE STIVIERE","CASTIGLIONE DELLE STIVIERE").gsub("D ALBA","D\'ALBA").gsub("D ADDA","D\'ADDA").gsub("D ASTI","D\'ASTI").gsub("SANT ELENA","SANT\'ELENA").gsub("L AQUILA","L\'AQUILA").gsub("D ESI","D\'ESI").gsub("DELL EMILIA","DELL\'EMILIA").gsub("FORLI","FORLI\'").gsub(" AURISINA","").gsub("CITTA ","CITTA\' ").gsub("SULL ARNO","SULL\'ARNO").gsub("IESOLO","JESOLO").gsub("SANT ANGELO","SANT\'ANGELO")',
...
#      'veh_marble_blocks_str'=> 'copy_field' ,
      'veh_model_str' => 'make=profilefield[:veh_model_str];make= make.gsub(/Â/,"" ) ; make= make.gsub(/°/,"a" ); make= make.gsub(/X6 \\(E71\\/E72\\) /,"X6 (E71)" ); make= make.gsub(/\\(/,"\\\\(" ); make= make.gsub(")","\\\\)" );regexpi_mod_array = make.split(" "); regexp_mod = "^(" + make + ")+|([A-Za-z0-9])*";regexpi_mod_array.each do |el|; regexp_mod = regexp_mod + "(" + el.to_s + ".*)+(\\\\s)*" end;regexp_mod = regexp_mod + "$";',
