...
#      'veh_set_up_str' => 'make=profilefield[:veh_set_up_str];make = make.gsub("/", "\\\\/").gsub("(", "\\\\(").gsub(")", "\\\\)"); make= make.gsub(/à/,"a" );make= make.gsub(/ì/,"i" );make= make.gsub(/ò/,"o" );make= make.gsub(/ù/,"u" ); make= make.gsub(/°/,"a" );  make= make.gsub(/(\\\\s)+ie(\\\\s)+/i, " i.e. "); regexpi_mod_array = make.split(" "); regexp_mod = "([A-Za-z0-9])*(\\\\s)*(?=.*?\\\\b" + regexpi_mod_array[0].to_s + "+" ; index=0 ;for index in (1..regexpi_mod_array.length-1) ;regexp_mod = regexp_mod + "((?=.*?\\\\b" + regexpi_mod_array[index].to_s + "\\\\b))*" end; regexp_mod = regexp_mod + ").+";',
        'veh_set_up_str' => 'make=profilefield[:veh_set_up_str];make = make.gsub("/", "\\\\/").gsub("(", "\\\\(").gsub(")", "\\\\)"); make= make.gsub(/à/,"a" );make= make.gsub(/ì/,"i" );make= make.gsub(/ò/,"o" );make= make.gsub(/ù/,"u" ); make= make.gsub(/°/,"a" );  make= make.gsub(/(\\\\s)+ie(\\\\s)+/i, " i.e. "); regexpi_mod_array = make.split(" "); regexp_mod = "([A-Za-z0-9])*" ;     regexpi_mod_array.each do |el|; regexp_mod = regexp_mod + "(" + el.to_s + ".*)+(\\\\s)*" end; regexp_mod = regexp_mod + "\\\\b|([A-Za-z0-9])*"; regexpi_mod_array = make.split(" "); regexpi_mod_array.each do |el|;el_array = el.scan(/\d+/);el_array.each do |num|; num = "(?=.*(" + num.to_s + ".*)+)(\\\\s)*"; regexp_mod = regexp_mod + num; end;el_array = el.scan(/\D+/); el_array.each do |char|;(char =~ /jt.*/i || char =~ /td.*/i || char =~ /cd.*/i || char =~ /sw|station|wagon/i )? char2 = "(?=.*(" + char.to_s + ".*)+)(\\\\s)*" : char2 = "(?=.*(" + char.to_s + ".*)*)(\\\\s)*";regexp_mod = regexp_mod + char2;end;end;regexp_mod = regexp_mod + "";',
