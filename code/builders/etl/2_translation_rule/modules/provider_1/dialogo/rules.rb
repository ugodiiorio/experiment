...
module Provider1Dialogo
  ...
  def build_hash_sect_1()
    ...
    @rule_values = {
      ...
      'veh_set_up_str' => 'kw=profilefield[:veh_kw_str].to_s;make=profilefield[:veh_set_up_str]; make= make.gsub(/à/,"a" ); make= make.gsub(/è/,"e" );make= make.gsub(/é/,"e" );make= make.gsub(/ì/,"i" );make= make.gsub(/ò/,"o" );make= make.gsub(/ù/,"u" ); make= make.gsub(/°/,"a" ); make= make.gsub(/ª/,"a" ); make= make.gsub(/tddi/i, "TD DI");  make= make.gsub(/tdci/i, "TD CI"); make= make.gsub(/5p./i, "5 PT");make= make.gsub(/3p./i, "3 PT");  make= make.gsub(/turbodiesel/i, "TD"); regexpi_mod_array = make.split(" "); regexp_mod = "regexpi:([A-Za-z0-9])*(\\\\s)*(?=.*?\\\\b" + regexpi_mod_array[0].to_s + "+" ; index=0 ;for index in (1..regexpi_mod_array.length-1);regexp_mod = regexp_mod + "((?=.*?\\\\b" + regexpi_mod_array[index].to_s + "\\\\b))*" end; regexp_mod = regexp_mod + ").+([A-Za-z0-9])*" + kw + ".* )";',
