...
#      'veh_set_up_str' => 'kw=profilefield[:veh_kw_num].to_s;make=profilefield[:veh_set_up_str]; make= make.gsub(/porte/,"PT" ); make= make.gsub(/turbodiesel/,"TD" );make= make.gsub(/weekend/,"WE" ); make= make.gsub(/raisin-top/,"R-TP" ); make= make.gsub(/twin spark/,"T-SK" ); make= make.gsub(/à/,"a" ); make= make.gsub(/è/,"e" );make= make.gsub(/é/,"e" );make= make.gsub(/ì/,"i" );make= make.gsub(/ò/,"o" );make= make.gsub(/ù/,"u" ); make= make.gsub(/°/,"a" ); make= make.gsub(/ª/,"a" ); make= make.gsub(/tddi/i, "TD DI");  make= make.gsub(/tdci/i, "TD CI"); make= make.gsub(/5p./i, "5 PT");make= make.gsub(/3p./i, "3 PT");  make= make.gsub(/turbodiesel/i, "TD"); make= make.gsub(/\\(/,"\\\\(" ); make= make.gsub(/\\)/,"\\\\)" ); make= make.gsub(/\\./,"\\\\." ); regexpi_mod_array = make.split(" ");regexp_mod = "(([A-Za-z0-9])*";regexpi_mod_array.each do |el|;el_array = el.scan(/\d+/);el_array.each do |num|;num = "(?=.*(" + num.to_s + ".*)+)(\\\\s)*";regexp_mod = regexp_mod + num;end;el_array = el.scan(/\D+/);el_array.each do |char|;(char =~ /jt.*/i || char =~ /td.*/i || char =~ /cd.*/i || char =~ /sw|station|wagon/i )? char2 = "(?=.*(" + char.to_s + ".*)+)(\\\\s)*" : char2 = "(?=.*(" + char.to_s + ".*)+)(\\\\s)*";regexp_mod = regexp_mod + char2;end;end;regexp_mod = regexp_mod + ").+(([A-Za-z0-9])*" + kw + ".* kw)|";regexp_mod = regexp_mod +"(([A-Za-z0-9])*(\\\\s)*(?=.*?\\\\b(" + regexpi_mod_array[0].to_s + ")*\\\\b)*" ;index=0 ;for index in (1..regexpi_mod_array.length-1);regexp_mod = regexp_mod + "((?=.*?\\\\b" + regexpi_mod_array[index].to_s + "\\\\b))*" end;regexp_mod = regexp_mod + ").+(([A-Za-z0-9])*" + kw + ".* kw)";',
        'veh_set_up_str' => 'kw=profilefield[:veh_kw_num].to_s;make=profilefield[:veh_set_up_str]; make= make.gsub(/porte/,"PT" ); make= make.gsub(/turbodiesel/,"TD" );make= make.gsub(/weekend/,"WE" ); make= make.gsub(/raisin-top/,"R-TP" ); make= make.gsub(/twin spark/,"T-SK" ); make= make.gsub(/à/,"a" ); make= make.gsub(/è/,"e" );make= make.gsub(/é/,"e" );make= make.gsub(/ì/,"i" );make= make.gsub(/ò/,"o" );make= make.gsub(/ù/,"u" ); make= make.gsub(/°/,"a" ); make= make.gsub(/ª/,"a" ); make= make.gsub(/tddi/i, "TD DI");  make= make.gsub(/tdci/i, "TD CI"); make= make.gsub(/5p./i, "5 PT");make= make.gsub(/3p./i, "3 PT");  make= make.gsub(/turbodiesel/i, "TD"); make= make.gsub(/\\(/,"\\\\(" ); make= make.gsub(/\\)/,"\\\\)" ); make= make.gsub(/\\./,"\\\\." );regexp_mod = "regexpi:([A-Za-z0-9])*("; regexpi_mod_array = make.split(" "); make.scan(/([0-9])+(\s)*(porte|pt|p(\.)*)+/i)[0].to_s == "" ? porte = "" : porte = "(?=.*(" + make.scan(/([0-9])+(\\s)*(porte|pt|p(\\.)*)+/i)[0].to_s + ".*)+)(\\\\s)*"; make = make.gsub(/([0-9])+(\\s)*(porte|pt|p(\\.)*)+/i, "");regexpi_mod_array = make.split(" ");regexpi_mod_array.each do |el|;if el =~ /([0-9]\\\\.[0-9])[a-z]{0,1}/i;( el = el.gsub(/([0-9]\\\\.[0-9])[a-z]{0,1}/, "\\\\1"); num = "(?=.*(" + el.to_s + ".*)+)(\\\\s)*" ; regexp_mod = regexp_mod + num;);   elsif el_array = el.scan(/\d+/); el_array  ? (el_array.each do |num|; num = "(?=.*(" + num.to_s + ".*)+)(\\\\s)*";regexp_mod = regexp_mod + num;end;el_array = el.scan(/\D+/);el_array.each do |char|;(char =~ /jt.*/i || char =~ /td.*/i || char =~ /cd.*/i || char =~ /sw|station|wagon/i )?char2 = "(?=.*(" + char.to_s + ".*)+)(\\\\s)*" : char2 = "(?=.*(" + char.to_s + ".*)*)(\\\\s)*";regexp_mod = regexp_mod + char2;end; ) : nil ;end; end;regexp_mod = regexp_mod + porte + ").+(([A-Za-z0-9])*(\\\\s)*(" + kw + " kw))|";regexp_mod = regexp_mod +"regexpi:(([A-Za-z0-9])*(\\\\s)*(?=.*?\\\\b(" + regexpi_mod_array[0].to_s + ")*\\\\b)*" ;index=0 ;for index in (1..regexpi_mod_array.length-1);regexp_mod = regexp_mod + "((?=.*?\\\\b" + regexpi_mod_array[index].to_s + "\\\\b))*" end;regexp_mod = regexp_mod + ").+(([A-Za-z0-9])*(\\\\s)*(" + kw + " kw))";',
#
...
#      'veh_set_up_str' => '"allestimento"',
      ...
      'own_owner_residence_str' =>  'res=profilefield[:own_owner_residence_str].gsub("VILLA D ALME","VILLA D\'ALME\'").gsub("PONTE SAN NICOLO","PONTE SAN NICOLO\'").gsub("DOLCE","DOLCE\'").gsub("FOSSO","FOSSO\'").gsub("ROMANO D EZZELINO","ROMANO D\'EZZELINO").gsub("SANT ANNA ARRESI","SANT\'ANNA ARRESI").gsub("RIVE D ARCANO","RIVE D\'ARCANO").gsub("CHALLAND SAINT ANSELME","CHALLAND-SAINT-ANSELME")',
      'driv_job_str' => 'job=profilefield[:driv_job_str]; spec=profilefield[:own_owner_specification_str] == "C" ? job = "impiegato" : job=job.gsub("militare", "forze dell\' ordine"); job=job.gsub("imprenditore / industriale", "libero professionista"); job=job.gsub("ingegnere", "libero professionista"); job=job.gsub("commerciante", "LAVORATORE INDIPENDENTE/AUTONOMO")  ; job=job.gsub("disoccupato", "senza occupazione")  ; job=job.gsub("artigiano", "LAVORATORE INDIPENDENTE/AUTONOMO")  ; job=job.gsub("medico", "ALTRE PROFESSIONI")  ;jregexp_mod = "regexpi:([A-Za-z0-9])*(";jregexpi_mod_array = job.split("/"); jregexpi_mod_array.each do |el| ;el_array= el.split(" "); el_array.each do |el2|; el2.length < 3 ? nil : jregexp_mod = jregexp_mod + "(" + el2.to_s + ".*)|" end;end ;jregexp_mod = jregexp_mod + "(niente))\\\\b"', 'pol_bersani_str' => 'translate_field',
      ...
      'pol_residence_str' => 'res=profilefield[:pol_residence_str].gsub("VILLA D ALME","VILLA D\'ALME\'").gsub("PONTE SAN NICOLO","PONTE SAN NICOLO\'").gsub("DOLCE","DOLCE\'").gsub("FOSSO","FOSSO\'").gsub("ROMANO D EZZELINO","ROMANO D\'EZZELINO").gsub("SANT ANNA ARRESI","SANT\'ANNA ARRESI").gsub("RIVE D ARCANO","RIVE D\'ARCANO").gsub("CHALLAND SAINT ANSELME","CHALLAND-SAINT-ANSELME")',
      ...
      'driv_birth_place_str' => 'res=profilefield[:driv_birth_place_str].gsub("VILLA D ALME","VILLA D\'ALME\'").gsub("PONTE SAN NICOLO","PONTE SAN NICOLO\'").gsub("DOLCE","DOLCE\'").gsub("FOSSO","FOSSO\'").gsub("ROMANO D EZZELINO","ROMANO D\'EZZELINO").gsub("SANT ANNA ARRESI","SANT\'ANNA ARRESI").gsub("RIVE D ARCANO","RIVE D\'ARCANO").gsub("CHALLAND SAINT ANSELME","CHALLAND-SAINT-ANSELME")',
