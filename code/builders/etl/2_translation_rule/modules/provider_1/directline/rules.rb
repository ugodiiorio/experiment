module Provider1Directline
  ...
  def build_hash_sect_1()
    ...
    @rule_values = {
      ...
      'veh_model_str' => 'make=profilefield[:veh_model_str];make= make.gsub(/---->/,"" );make= make.gsub(/--->/,"" );make= make.gsub(/à/,"a" ); make= make.gsub(/è/,"e" );make= make.gsub(/é/,"e" );make= make.gsub(/ì/,"i" );make= make.gsub(/ò/,"o" );make= make.gsub(/ù/,"u" ); make= make.gsub(/°/,"a" );  make= make.gsub(/ª/i, "a"); make= make.gsub(/ª/, "a "); regexpi_mod_array = make.split(" "); regexp_mod = "regexpi:([A-Za-z0-9])*(" + make + ")+|";regexpi_mod_array.each do |el|; regexp_mod = regexp_mod + "(" + el.to_s + ")+(\\\\s)*" end;regexp_mod = regexp_mod + "$";',
