...
#      'veh_marble_blocks_str'=> 'copy_field',
      'veh_model_str' => 'make=profilefield[:veh_model_str];make= make.gsub(/---->/,"-----" );make= make.gsub(/--->/,"----" ); make= make.gsub(/->/,"" ); make= make.gsub(/à/,"a" ); make= make.gsub(/è/,"e" );make= make.gsub(/é/,"e" );make= make.gsub(/ì/,"i" );make= make.gsub(/ò/,"o" );make= make.gsub(/ù/,"u" ); make= make.gsub(/°/,"a" );  make= make.gsub(/ª/i, "a"); make= make.gsub(/ª/, "a ");  make= make.gsub(/\\(/,"\\\\(" ); make= make.gsub(/\\)/,"\\\\)" ); make= make.gsub(/X6 \\(E71\\)/,"X6 \\(E71-E72\\)" ); regexpi_mod_array = make.split(" "); regexp_mod = "regexpi:^(" + make + ")+|regexpi:^";regexpi_mod_array.each do |el|; regexp_mod = regexp_mod + "(" + el.to_s + ".*)+(\\\\s)*" end;regexp_mod = regexp_mod + "$";',
