...
#      'driv_birth_place_str' => 'res = profilefield[:driv_birth_place_str].upcase.strip;res = res.gsub("PORTO SANT ELPIDIO","PORTO SANT\'ELPIDIO");res.gsub("VILLA D ALME", "VILLA D\'ALME");res.gsub("TORRE DE PASSERI", "TORRE DE\' PASSERI");res.gsub("S CASCIANO IN VAL DI PESA", "SAN CASCIANO IN VAL DI PESA");res.gsub("DOLCE","DOLCE\'");',
...
#      'pol_BM_1_more_than_1_year_str' => 'copy_field',
      'pol_BM_assigned_str' => 'profilefield[:pol_bersani_str] == "no" ? translate_field : ( profilefield[:pol_BM_assigned_str].to_i == -1 ? "CLASSE01" : translate_field)',
...
#      'pol_residence_province_str' => 'translate_field',
      'pol_residence_str' => 'res=profilefield[:pol_residence_str].gsub("D ","D\'").gsub("SANT ","SANT\'").gsub("SULL ","SULL\'").gsub("CASTELL ","CASTELL\'").gsub("MASERA","MASERA\'").gsub("S CASCIANO","SAN CASCIANO IN").gsub("LAVENO ","LAVENO-").gsub("VILLANOVA CAMPOSAMPIERO","VILLANOVA DI CAMPOSAMPIERO").gsub("DUINO ","DUINO-").gsub("MONTECATINI TERME","MONTECATINI-TERME").gsub("CASTROCARO TERRA SOLE","CASTROCARO TERME TERRA SOLE").gsub("S CASCIANO IN","SAN CASCIANO").gsub("UGGIATE TREVANO","UGGIATE-TREVANO").gsub("TOSCOLANO MADERNO","TOSCOLANO-MADERNO").gsub("BELLARIA IGEA MARINA","BELLARIA-IGEA MARINA").gsub("ROVERE VERONESE","ROVERE\' VERONESE").gsub("SANT ANNA","SANT\'ANNA").gsub("D EZZELINO","D\'EZZELINO").gsub("D ALME", "D\'ALME").gsub("DE PASSERI", "DE\' PASSERI").gsub("S CASCIANO", "SAN CASCIANO").gsub("REGGIO NELL\'EMILIA","REGGIO EMILIA").gsub("SANT ELPIDIO","SANT\'ELPIDIO").gsub("DOLCE","DOLCE\'").gsub("D ARCANO","D\'ARCANO").gsub("CASTIGLIONE STIVIERE","CASTIGLIONE DELLE STIVIERE").gsub("BRISSAGO VALTRAVAGLIA","BRISSAGO-VALTRAVAGLIA").gsub("D ALBA","D\'ALBA").gsub("D ADDA","D\'ADDA").gsub("D ASTI","D\'ASTI").gsub("COCQUIO TREVISAGO","COCQUIO-TREVISAGO").gsub("SANT ELENA","SANT\'ELENA").gsub("L AQUILA","L\'AQUILA").gsub("D ESI","D\'ESI").gsub(" FABIASCO","-FABIASCO").gsub("DELL EMILIA","DELL\'EMILIA").gsub("SCORZE","SCORZE\'").gsub("MONSUE","MONSUE\'").gsub("CITTA","CITTA\'")',
