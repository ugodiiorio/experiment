module Provider1Genialloyd
  ...
  def build_hash_sect_1()
    ...
    @rule_values = {
      ...
      'veh_set_up_str' => 'make=profilefield[:veh_set_up_str];make= make.gsub(/\//,"-" ); make= make.gsub(/à/,"a" ); make= make.gsub(/è/,"e" );make= make.gsub(/é/,"e" );make= make.gsub(/ì/,"i" );make= make.gsub(/ò/,"o" );make= make.gsub(/ù/,"u" ); make= make.gsub(/°/,"a" ); make= make.gsub(/benzina/i ,"bz." );make= make.gsub(/turbodiesel/i ,"TD" );make= make.gsub(/ª/,"a" );  make= make.gsub(/3 porte/i, "3p "); make= make.gsub(/5 porte/i, "5p ");regexpi_mod_array = make.split(" "); regexp_mod = "regexpi:([A-Za-z0-9])*";regexpi_mod_array.each do |el|; regexp_mod = regexp_mod + "(" + el.to_s + ".*)+(\\\\s)*" end;regexp_mod = regexp_mod + "$";',
