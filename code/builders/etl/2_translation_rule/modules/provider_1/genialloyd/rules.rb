...
#      'driv_birth_province_str' => 'copy_field',
      'driv_birth_state_str' => 'state=profilefield[:driv_birth_state_str].gsub("GRAN BRETAGNA","GRAN BRETAGNA IRL. NORD").gsub("CECOSLOVACCHIA","REPUBBLICA CECA").gsub("PERU","PERU\' SURINAME").gsub("IUGOSLAVIA","JUGOSLAVIA")',
      'driv_citizenship_str' => 'state=profilefield[:driv_citizenship_str].gsub("GRAN BRETAGNA","GRAN BRETAGNA IRL. NORD").gsub("CECOSLOVACCHIA","REPUBBLICA CECA").gsub("PERU","PERU\' SURINAME").gsub("IUGOSLAVIA","JUGOSLAVIA")',
...
#      'driv_job_2_str' => 'copy_field',
      'driv_job_str' => 'job=profilefield[:driv_job_str]; job=job.gsub("imprenditore", "altre professioni" ).gsub("ecclesiastico", "altre professioni" ).gsub("agricoltore", "altre professioni" ).gsub("studente", "altre professioni" ).gsub("altri", "altre professioni" ).gsub("autista professionista", "altre professioni" ).gsub("artigiano", "altre professioni" ).gsub("ingegnere", "altre professioni" ).gsub("libero professionista", "altre professioni" ).gsub("medico", "altre professioni" ).gsub("militare", "Forze Armate" ).gsub("insegnante", "impiegato" ).gsub("disoccupato", "Senza occupazione").gsub("pensionato da lavoro", "Altre professioni").gsub("casalinga", "Altre professioni").gsub("libero professionista", "Altre professioni").gsub("imprenditore / industriale", "Altre professioni").gsub("commerciante", "Altre professioni");jregexp_mod = "regexpi:([A-Za-z0-9])*(";jregexpi_mod_array = job.split("/"); jregexpi_mod_array.each do |el| ;el_array= el.split(" "); el_array.each do |el2|; el2.length < 3 ? nil : jregexp_mod = jregexp_mod + "(" + el2.to_s + ".*)|" end;end ;jregexp_mod = jregexp_mod + "(niente))\\\\b"',
...
#      'pol_nr_of_paid_claims_2_yr_str' => 'copy_field',
      'pol_nr_of_paid_claims_1_yr_str' => 'x= profilefield[:pol_nr_of_paid_claims_this_yr_str].to_i; y= profilefield[:pol_nr_of_paid_claims_1_yr_str].to_i; sin= x+y; if sin == 0; sin ="Nessuno"; elsif sin == 1; sin ="Uno"; elsif sin == 2; sin ="Due";  else "Più di due" end ',
