...
'veh_model_str' => 'make=infield[:veh_model_str]; make= make.gsub(/ª/, ""a""); regexpi_mod_array = make.split("" ""); regexp_mod = ""regexpi:([A-Za-z0-9])*"";regexpi_mod_array.each do |el|; regexp_mod = regexp_mod + ""("" + el.to_s + "".*)+(\\\\s)*"" end;regexp_mod = regexp_mod + ""\\\\b"";',
'veh_set_up_str' => 'make=infield[:veh_set_up_str]; make= make.gsub(/td/i, ""turbodiesel""); make= make.gsub(/(?![0-9]+)p(\\s)+/i, "" porte""); regexpi_mod_array = make.split("" ""); regexp_mod = ""regexpi:([A-Za-z0-9])*"";regexpi_mod_array.each do |el|; regexp_mod = regexp_mod + ""("" + el.to_s + "".*)+(\\\\s)*"" end;regexp_mod = regexp_mod + ""\\\\b"";',
...
'pol_nr_of_paid_claims_2_yr_str' => 'x= profilefield[:pol_nr_of_paid_claims_this_yr_str].to_i; y= profilefield[: pol_nr_of_paid_claims_1_yr_str].to_i; sin= x+y; if sin == 0; sin =”Nessuno”; elsif sin == 1; sin =”Uno”; sin == 2; sin =”Due”; ',
'pol_bersani_ref_vehicle_number_plate_str' => 'translate_field',
'pol_claims_total_number_str' => 'translate_field',
'pol_driver_sex_str' => 'copy_field',
