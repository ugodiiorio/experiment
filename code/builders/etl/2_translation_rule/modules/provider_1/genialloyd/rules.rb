module Provider1Genialloyd
  ...
  def build_hash_sect_1()
    ...
    @rule_values = {
      ...
      'driv_birth_state_str' => 'state=profilefield[:driv_birth_state_str].gsub("GRAN BRETAGNA","GRAN BRETAGNA IRL. NORD").gsub("CECOSLOVACCHIA","REPUBBLICA CECA").gsub("PERU","PERU\'").gsub("IUGOSLAVIA","JUGOSLAVIA")',
      'driv_citizenship_str' => 'state=profilefield[:driv_citizenship_str].gsub("GRAN BRETAGNA","GRAN BRETAGNA IRL. NORD").gsub("CECOSLOVACCHIA","REPUBBLICA CECA").gsub("PERU","PERU\'").gsub("IUGOSLAVIA","JUGOSLAVIA")',
      ...
      'driv_job_str' => 'job=profilefield[:driv_job_str]; job=job.gsub("imprenditore", "altre professioni" ); job=job.gsub("ecclesiastico", "altre professioni" ); job=job.gsub("agricoltore", "altre professioni" );job=job.gsub("studente", "altre professioni" ); job=job.gsub("altri", "altre professioni" ); job=job.gsub("autista professionista", "altre professioni" );job=job.gsub("artigiano", "altre professioni" ); job=job.gsub("ingegnere", "altre professioni" );  job=job.gsub("libero professionista", "altre professioni" ); job=job.gsub("medico", "altre professioni" ); job=job.gsub("militare", "forze armate" ); job=job.gsub("insegnante", "impiegato" ); job=job.gsub("disoccupato", "senza occupazione" );jregexp_mod = "regexpi:([A-Za-z0-9])*(";jregexpi_mod_array = job.split("/"); jregexpi_mod_array.each do |el| ;el_array= el.split(" "); el_array.each do |el2|; el2.length < 3 ? nil : jregexp_mod = jregexp_mod + "(" + el2.to_s + ".*)|" end;end ;jregexp_mod = jregexp_mod + "(niente))\\\\b"',
      ...
      'pol_assistance_premium_id_str' => 'translate_field',
      ...
      'pol_nr_of_paid_claims_1_yr_str' => 'x= profilefield[:pol_nr_of_paid_claims_this_yr_str].to_i; y= profilefield[:pol_nr_of_paid_claims_1_yr_str].to_i; sin= x+y; if sin == 0; sin ="Nessuno"; elsif sin == 1; sin ="Uno"; elsif sin == 2; sin ="Due" end ',
      ...
      #      'veh_set_up_str' => 'make=profilefield[:veh_set_up_str];make= make.gsub(/\\./,"\\\\." ); make= make.gsub(/\//,"-" ); make= make.gsub(/à/,"a" ); make= make.gsub(/ì/,"i" );make= make.gsub(/ò/,"o" );make= make.gsub(/ù/,"u" ); make= make.gsub(/°/,"a" ); make= make.gsub(/benzina/i ,"bz." );make= make.gsub(/turbodiesel/i ,"TD" );make= make.gsub(/ª/,"a" );  make= make.gsub(/3 porte/i, "3p "); make= make.gsub(/5 porte/i, "5p ");regexpi_mod_array = make.split(" "); regexp_mod = "regexpi:([A-Za-z0-9])*";regexpi_mod_array.each do |el|; regexp_mod = regexp_mod + "(" + el.to_s + ".*)+(\\\\s)*" end;regexp_mod = regexp_mod + "$|regexpi:([A-Za-z0-9])*"; regexpi_mod_array = make.split(" "); regexpi_mod_array.each do |el|;el_array = el.scan(/\d+/);el_array.each do |num|; num = "(?=.*(" + num.to_s + ".*)+)(\\\\s)*"; regexp_mod = regexp_mod + num; end;el_array = el.scan(/\D+/); el_array.each do |char|;(char =~ /jt.*/i || char =~ /td.*/i || char =~ /cd.*/i || char =~ /sw|station|wagon/i )? char2 = "(?=.*(" + char.to_s + ".*)+)(\\\\s)*" : char2 = "(?=.*(" + char.to_s + ".*)*)(\\\\s)*";regexp_mod = regexp_mod + char2;end;end;regexp_mod = regexp_mod + "";',
      'veh_set_up_str' => 'make=profilefield[:veh_set_up_str];make= make.gsub(/\\./,"\\\\." ); make= make.gsub(/\\(/,"\\\\(" ); make= make.gsub(/\\)/,"\\\\)" );make= make.gsub(/([0-9][0-9][0-9])([a-z])/i, "\\\\1(\\\\s)*\\\\2"); make= make.gsub(/\//,"-" ); make= make.gsub(/è/,"e" ); make= make.gsub(/é/,"e" );make= make.gsub(/à/,"a" ); make= make.gsub(/ì/,"i" );make= make.gsub(/ò/,"o" );make= make.gsub(/ù/,"u" ); make= make.gsub(/°/,"a" ); make= make.gsub(/benzina/i ,"bz." );make= make.gsub(/turbodiesel/i ,"TD" );make= make.gsub(/ª/,"a" );  make= make.gsub(/3 porte/i, "3p "); make= make.gsub(/4 porte/i, "4p "); make= make.gsub(/5 porte/i, "5p "); make= make.gsub(/(3|4|5)p\\\\./i, "\\\\1p ");regexp_mod = "regexpi:([A-Za-z0-9])*" ; regexpi_mod_array = make.split(" "); regexpi_mod_array.each do |el|; regexp_mod= regexp_mod + "(" + el + ".*)+(\\\\s)*"; end ; regexp_mod = regexp_mod + "|regexpi:([A-Za-z0-9])*"; regexpi_mod_array = make.split(" "); make.scan(/([0-9])+(\s)*(porte|pt|p(\.)*)+/i)[0].to_s == "" ? porte = "" : porte = "(?=.*(" + make.scan(/([0-9])+(\\s)*(porte|pt|p(\\.)*)+/i)[0].to_s + ".*)+)(\\\\s)*"; make = make.gsub(/([0-9])+(\\s)*(porte|pt|p(\\.)*)+/i, "");regexpi_mod_array = make.split(" ");regexpi_mod_array.each do |el|;if el =~ /([0-9]\\\\.[0-9])[a-z]{0,1}/i;( el = el.gsub(/([0-9]\\\\.[0-9])[a-z]{0,1}/, "\\\\1"); num = "(?=.*(" + el.to_s + ".*)+)(\\\\s)*" ; regexp_mod = regexp_mod + num;); elsif el =~ /([0-9]{1,3})(\\\\s)*(cv|v)/i; num = "(?=.*(" + el.to_s + ".*)+)(\\\\s)*" ; regexp_mod = regexp_mod + num;  elsif el_array = el.scan(/\d+/); el_array  ? (el_array.each do |num|; num = "(?=.*(" + num.to_s + ".*)+)(\\\\s)*";regexp_mod = regexp_mod + num;end;el_array = el.scan(/\D+/);el_array.each do |char|;(char =~ /jt.*/i || char =~ /td.*/i || char =~ /cd.*/i || char =~ /sw|station|wagon|(s\\.w\\.)/i )?char2 = "(?=.*(" + char.to_s + ".*)+)(\\\\s)*" : char2 = "(?=.*(" + char.to_s + ".*)*)(\\\\s)*";regexp_mod = regexp_mod + char2;end; ) : nil ;end; end;regexp_mod = regexp_mod + porte + "";',
