module Provider1Genialloyd
  ...
  def build_hash_sect_1()
    ...
    @rule_values = {
      ...
      'driv_job_str' => 'job=profilefield[:driv_job_str]; job=job.gsub("imprenditore", "altre professioni" ); job=job.gsub("studente", "altre professioni" ); job=job.gsub("altri", "altre professioni" ); job=job.gsub("autista professionista", "altre professioni" );job=job.gsub("artigiano", "altre professioni" ); job=job.gsub("libero professionista", "altre professioni" ); job=job.gsub("medico", "altre professioni" ); job=job.gsub("militare", "forze armate" ); job=job.gsub("insegnante", "impiegato" ); job=job.gsub("disoccupato", "senza occupazione" );jregexp_mod = "regexpi:([A-Za-z0-9])*(";jregexpi_mod_array = job.split("/"); jregexpi_mod_array.each do |el| ;el_array= el.split(" "); el_array.each do |el2|; el2.length < 3 ? nil : jregexp_mod = jregexp_mod + "(" + el2.to_s + ".*)|" end;end ;jregexp_mod = jregexp_mod + "(niente))\\\\b"',
