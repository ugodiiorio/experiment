module Provider1Genialloyd
  ...
  def build_hash_sect_1()
    ...
    @rule_values = {
      ...
      'veh_set_up_str' => 'make=profilefield[:veh_set_up_str];make= make.gsub(/\\./,"\\\\." ); make= make.gsub(/([0-9][0-9][0-9])([a-z])/i, "\\1(\\s)*\\2"); make= make.gsub(/\//,"-" ); make= make.gsub(/è/,"e" ); make= make.gsub(/é/,"e" );make= make.gsub(/à/,"a" ); make= make.gsub(/ì/,"i" );make= make.gsub(/ò/,"o" );make= make.gsub(/ù/,"u" ); make= make.gsub(/°/,"a" ); make= make.gsub(/benzina/i ,"bz." );make= make.gsub(/turbodiesel/i ,"TD" );make= make.gsub(/ª/,"a" );  make= make.gsub(/3 porte/i, "3p "); make= make.gsub(/5 porte/i, "5p ");regexpi_mod_array = make.split(" "); regexp_mod = "regexpi:([A-Za-z0-9])*(" + make + ")+|regexpi:([A-Za-z0-9])*"; regexpi_mod_array = make.split(" "); regexpi_mod_array.each do |el|;el_array = el.scan(/\d+/);el_array.each do |num|; num = "(?=.*(" + num.to_s + ".*)+)(\\\\s)*"; regexp_mod = regexp_mod + num; end;el_array = el.scan(/\D+/); el_array.each do |char|;(char =~ /jt.*/i || char =~ /td.*/i || char =~ /cd.*/i || char =~ /sw|station|wagon/i )? char2 = "(?=.*(" + char.to_s + ".*)+)(\\\\s)*" : char2 = "(?=.*(" + char.to_s + ".*)*)(\\\\s)*";regexp_mod = regexp_mod + char2;end;end;regexp_mod = regexp_mod + "";',
