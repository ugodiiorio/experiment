...
#      'veh_set_up_str' => 'make=profilefield[:veh_set_up_str];make= make.gsub(/à/,"a" );make= make.gsub(/ì/,"i" );make= make.gsub(/ù/,"u" ); make= make.gsub(/°/,"a" ); make= make.gsub(/\\./,"\\\\." ); make= make.gsub(/\\(/,"\\\\(" ); make= make.gsub(/\\)/,"\\\\)" );  make= make.gsub(/(\\s)+ie(\\s)+/i, " i.e. ");  regexpi_mod_array = make.split(" "); regexp_mod = "regexpi:([A-Za-z0-9])*";regexpi_mod_array.each do |el|; regexp_mod = regexp_mod + "(" + el.to_s + ".*)+(\\\\s)*" end;regexp_mod = regexp_mod + "$|regexpi:([A-Za-z0-9])*"; regexpi_mod_array = make.split(" "); regexpi_mod_array.each do |el|;el_array = el.scan(/\d+/);el_array.each do |num|; num = "(?=.*(" + num.to_s + ".*)+)(\\\\s)*"; regexp_mod = regexp_mod + num; end;el_array = el.scan(/\D+/); el_array.each do |char|;(char =~ /jt.*/i || char =~ /td.*/i || char =~ /cd.*/i || char =~ /sw|station|wagon/i )? char2 = "(?=.*(" + char.to_s + ".*)+)(\\\\s)*" : char2 = "(?=.*(" + char.to_s + ".*)*)(\\\\s)*";regexp_mod = regexp_mod + char2;end;end;regexp_mod = regexp_mod + "";',
    'veh_set_up_str' => 'make=profilefield[:veh_set_up_str];make= make.gsub(/à/,"a" );make= make.gsub(/ì/,"i" );make= make.gsub(/ù/,"u" ); make= make.gsub(/°/,"a" ); make= make.gsub(/\\./,"\\\\." ); make= make.gsub(/\\(/,"\\\\(" ); make= make.gsub(/\\)/,"\\\\)" );  make= make.gsub(/(\\s)+ie(\\s)+/i, " i.e. ");  regexp_mod = "regexpi:([A-Za-z0-9])*" ; regexpi_mod_array = make.split(" "); regexpi_mod_array.each do |el|; regexp_mod= regexp_mod + "(" + el + ".*)+(\\\\s)*"; end ; regexp_mod = regexp_mod + "|regexpi:([A-Za-z0-9])*"; regexpi_mod_array = make.split(" "); make.scan(/([0-9])+(\s)*(porte|pt|p(\.)*)+/i)[0].to_s == "" ? porte = "" : porte = "(?=.*(" + make.scan(/([0-9])+(\\s)*(porte|pt|p(\\.)*)+/i)[0].to_s + ".*)+)(\\\\s)*"; make = make.gsub(/([0-9])+(\\s)*(porte|pt|p(\\.)*)+/i, "");regexpi_mod_array = make.split(" ");regexpi_mod_array.each do |el|;if el =~ /([0-9]\\\\.[0-9])[a-z]{0,1}/i;( el = el.gsub(/([0-9]\\\\.[0-9])[a-z]{0,1}/, "\\\\1"); num = "(?=.*(" + el.to_s + ".*)+)(\\\\s)*" ; regexp_mod = regexp_mod + num;);   elsif el_array = el.scan(/\d+/); el_array  ? (el_array.each do |num|; num = "(?=.*(" + num.to_s + ".*)+)(\\\\s)*";regexp_mod = regexp_mod + num;end;el_array = el.scan(/\D+/);el_array.each do |char|;(char =~ /jt.*/i || char =~ /td.*/i || char =~ /cd.*/i || char =~ /sw|station|wagon/i )?char2 = "(?=.*(" + char.to_s + ".*)+)(\\\\s)*" : char2 = "(?=.*(" + char.to_s + ".*)*)(\\\\s)*";regexp_mod = regexp_mod + char2;end; ) : nil ;end; end;regexp_mod = regexp_mod + porte + "";',
      ...
      'pol_residence_str' => 'res=profilefield[:pol_residence_str].gsub("PONTE SAN NICOLO","PONTE SAN NICOLO\'").gsub("FOSSO","FOSSO\'").gsub("ROMANO D EZZELINO","ROMANO D\'EZZELINO").gsub("VILLA D ALME","VILLA D\'ALME\'").gsub("TORRE DE PASSERI","TORRE DE\' PASSERI").gsub("S CASCIANO IN VAL DI PESA","SAN CASCIANO IN VAL DI PESA").gsub("REGGIO NELL\'EMILIA","REGGIO EMILIA").gsub("PORTO SANT ELPIDIO","PORTO SANT\'ELPIDIO").gsub("DOLCE","DOLCE\'").gsub("SANT ANNA ARRESI","SANT\'ANNA ARRESI").gsub("ROVERE VERONESE","ROVERE\' VERONESE").gsub("RIVE D ARCANO","RIVE D\'ARCANO").gsub("CASTIGLIONE STIVIERE","CASTIGLIONE DELLE STIVIERE").gsub("MUGGIO","MUGGIO\'");',
      'own_owner_specification_str' => 'copy_field',
      'own_owner_residence_province_str' =>  'translate_field'   ,
      'own_owner_residence_str' => 'res=profilefield[:own_owner_residence_str].gsub("PONTE SAN NICOLO","PONTE SAN NICOLO\'").gsub("FOSSO","FOSSO\'").gsub("ROMANO D EZZELINO","ROMANO D\'EZZELINO").gsub("VILLA D ALME","VILLA D\'ALME\'").gsub("TORRE DE PASSERI","TORRE DE\' PASSERI").gsub("S CASCIANO IN VAL DI PESA","SAN CASCIANO IN VAL DI PESA").gsub("REGGIO NELL\'EMILIA","REGGIO EMILIA").gsub("PORTO SANT ELPIDIO","PORTO SANT\'ELPIDIO").gsub("DOLCE","DOLCE\'").gsub("SANT ANNA ARRESI","SANT\'ANNA ARRESI").gsub("ROVERE VERONESE","ROVERE\' VERONESE").gsub("RIVE D ARCANO","RIVE D\'ARCANO").gsub("CASTIGLIONE STIVIERE","CASTIGLIONE DELLE STIVIERE").gsub("MUGGIO","MUGGIO\'");',
...
#      'pol_matriculation_date_day_str' => 'profilefield[:pol_matriculation_date_str] == 10 ? copy_field : (Chronic.parse(@rate_date) - profilefield[:pol_matriculation_date_str].to_i.years).strftime("%d")',
      ...
      'driv_birth_place_str' => 'res=profilefield[:driv_birth_place_str].gsub("PONTE SAN NICOLO","PONTE SAN NICOLO\'").gsub("FOSSO","FOSSO\'").gsub("ROMANO D EZZELINO","ROMANO D\'EZZELINO").gsub("VILLA D ALME","VILLA D\'ALME\'").gsub("TORRE DE PASSERI","TORRE DE\' PASSERI").gsub("S CASCIANO IN VAL DI PESA","SAN CASCIANO IN VAL DI PESA").gsub("REGGIO NELL\'EMILIA","REGGIO EMILIA").gsub("PORTO SANT ELPIDIO","PORTO SANT\'ELPIDIO").gsub("DOLCE","DOLCE\'").gsub("SANT ANNA ARRESI","SANT\'ANNA ARRESI").gsub("ROVERE VERONESE","ROVERE\' VERONESE").gsub("RIVE D ARCANO","RIVE D\'ARCANO").gsub("CASTIGLIONE STIVIERE","CASTIGLIONE DELLE STIVIERE").gsub("MUGGIO","MUGGIO\'");',
