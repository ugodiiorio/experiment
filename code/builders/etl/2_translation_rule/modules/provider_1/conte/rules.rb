...
'veh_model_str' => 'make=infield[:veh_model_str];  regexpi_mod_array = make.split(" "); regexp_mod = "regexpi:([A-Za-z0-9])*";regexpi_mod_array.each do |el|; regexp_mod = regexp_mod + "(" + el.to_s + ".*)+(\\\\s)*" end;regexp_mod = regexp_mod + "\\\\b";',
'veh_set_up_str' => 'make=infield[:veh_set_up_str]; make= make.gsub(/(\\s)+ie(\\s)+/i, " i.e. ");  regexpi_mod_array = make.split(" "); regexp_mod = "regexpi:([A-Za-z0-9])*";regexpi_mod_array.each do |el|; regexp_mod = regexp_mod + "(" + el.to_s + ".*)+(\\\\s)*" end;regexp_mod = regexp_mod + "\\\\b";',
...
'driv_birth_state_str' => 'translate_field',
'pol_minikasko_str' => 'copy_field',
'driv_job_str' => 'ob=infield[:driv_job_str]; jregexp_mod = "regexpi:([A-Za-z0-9])*";jregexpi_mod_array = job.split("/"); jregexpi_mod_array.each do |el| ; el.split("/"); el.each do |el2|; jregexp_mod = jregexp_mod + "(" + el2.to_s + ".*)*(\\\\s)*" end; end ; jregexp_mod = jregexp_mod + "\\\\b" ;',
...
'veh_purchase_date_str' => 'data= Date.parse(profilefield[:pol_matriculation_date_str]);european_date = \'%m/%Y\';data = data.strftime(european_date).to_s',
...
'pol_act_of_vandalism_str' => 'translate_field',
'pol_on_off_act_of_vandalism_str' => 'translate_field',
'pol_kasko_minimum_uncoverage_str' => 'translate_field',
'pol_kasko_str' => 'translate_field',
'pol_natural_events_str' => 'copy_field',
...
'pol_nr_of_paid_claims_2_yr_str' => 'x= profilefield[:pol_nr_of_paid_claims_this_yr_str].to_i; y= profilefield[: pol_nr_of_paid_claims_1_yr_str].to_i; z= profilefield[: pol_nr_of_paid_claims_2_yr_str].to_i; sin= x+y+z',
