...
#      'veh_set_up_str' => 'make=profilefield[:veh_set_up_str];make= make.gsub(/à/,"a" );make= make.gsub(/ì/,"i" );make= make.gsub(/ù/,"u" ); make= make.gsub(/°/,"a" ); make= make.gsub(/\\./,"\\\\." ); make= make.gsub(/\\(/,"\\\\(" ); make= make.gsub(/\\)/,"\\\\)" );  make= make.gsub(/(\\s)+ie(\\s)+/i, " i.e. ");  regexpi_mod_array = make.split(" "); regexp_mod = "regexpi:([A-Za-z0-9])*";regexpi_mod_array.each do |el|; regexp_mod = regexp_mod + "(" + el.to_s + ".*)+(\\\\s)*" end;regexp_mod = regexp_mod + "$|regexpi:([A-Za-z0-9])*"; regexpi_mod_array = make.split(" "); regexpi_mod_array.each do |el|;el_array = el.scan(/\d+/);el_array.each do |num|; num = "(?=.*(" + num.to_s + ".*)+)(\\\\s)*"; regexp_mod = regexp_mod + num; end;el_array = el.scan(/\D+/); el_array.each do |char|;(char =~ /jt.*/i || char =~ /td.*/i || char =~ /cd.*/i || char =~ /sw|station|wagon/i )? char2 = "(?=.*(" + char.to_s + ".*)+)(\\\\s)*" : char2 = "(?=.*(" + char.to_s + ".*)*)(\\\\s)*";regexp_mod = regexp_mod + char2;end;end;regexp_mod = regexp_mod + "";',
    'veh_set_up_str' => 'make=profilefield[:veh_set_up_str];make= make.gsub(/à/,"a" );make= make.gsub(/ì/,"i" );make= make.gsub(/ù/,"u" ); make= make.gsub(/°/,"a" ); make= make.gsub(/\\./,"\\\\." ); make= make.gsub(/\\(/,"\\\\(" ); make= make.gsub(/\\)/,"\\\\)" );  make= make.gsub(/(\\s)+ie(\\s)+/i, " i.e. ");  regexp_mod = "regexpi:([A-Za-z0-9])*" ; regexpi_mod_array = make.split(" "); regexpi_mod_array.each do |el|; regexp_mod= regexp_mod + "(" + el + ".*)+(\\\\s)*"; end ; regexp_mod = regexp_mod + "|regexpi:([A-Za-z0-9])*"; regexpi_mod_array = make.split(" "); make.scan(/([0-9])+(\s)*(porte|pt|p(\.)*)+/i)[0].to_s == "" ? porte = "" : porte = "(?=.*(" + make.scan(/([0-9])+(\\s)*(porte|pt|p(\\.)*)+/i)[0].to_s + ".*)+)(\\\\s)*"; make = make.gsub(/([0-9])+(\\s)*(porte|pt|p(\\.)*)+/i, "");regexpi_mod_array = make.split(" ");regexpi_mod_array.each do |el|;if el =~ /([0-9]\\\\.[0-9])[a-z]{0,1}/i;( el = el.gsub(/([0-9]\\\\.[0-9])[a-z]{0,1}/, "\\\\1"); num = "(?=.*(" + el.to_s + ".*)+)(\\\\s)*" ; regexp_mod = regexp_mod + num;);  elsif el =~ /([0-9]{1,3})(\\\\s)*(cv|v)/i; num = "(?=.*(" + el.to_s + ".*)+)(\\\\s)*" ; regexp_mod = regexp_mod + num; elsif el_array = el.scan(/\d+/); el_array  ? (el_array.each do |num|; num = "(?=.*(" + num.to_s + ".*)+)(\\\\s)*";regexp_mod = regexp_mod + num;end;el_array = el.scan(/\D+/);el_array.each do |char|;(char =~ /jt.*/i || char =~ /td.*/i || char =~ /cd.*/i || char =~ /sw|station|wagon/i )?char2 = "(?=.*(" + char.to_s + ".*)+)(\\\\s)*" : char2 = "(?=.*(" + char.to_s + ".*)*)(\\\\s)*";regexp_mod = regexp_mod + char2;end; ) : nil ;end; end;regexp_mod = regexp_mod + porte + "";',
      ...
      'driv_birth_place_str' => 'res=profilefield[:driv_birth_place_str].gsub("SAN NICOLO","SAN NICOLO\'").gsub("FOSSO","FOSSO\'").gsub("D EZZELINO","D\'EZZELINO").gsub("D ALBA","D\'ALBA").gsub("L AQUILA","L\'AQUILA").gsub("CITTA ","CITTA\'").gsub("D ALME","D\'ALME\'").gsub("SULL ARNO","SULL\' ARNO").gsub("DE PASSERI","DE\' PASSERI").gsub("S CASCIANO","SAN CASCIANO").gsub("REGGIO NELL\'EMILIA","REGGIO EMILIA").gsub("SANT ELPIDIO","SANT\'ELPIDIO").gsub("DOLCE","DOLCE\'").gsub("SANT ANNA","SANT\'ANNA").gsub("ROVERE VERONESE","ROVERE\' VERONESE").gsub("D ARCANO","D\'ARCANO").gsub("CASTIGLIONE STIVIERE","CASTIGLIONE DELLE STIVIERE").gsub("MUGGIO","MUGGIO\'").gsub("D ASTI","D\'ASTI").gsub("SANT ELENA","SANT\'ELENA").gsub("D ESI","D\'ESI").gsub("D ADDA","D\'ADDA").gsub("DELL EMILIA","DELL\'EMILIA").gsub("MANSUE","MANSUE\'").gsub("FORLI","FORLI\'").gsub("SANT ANGELO","SANT\'ANGELO").gsub("SANTA MARIA DELLE MOLE","MARINO").gsub("FARA IN SABINA","FARA SABINA").gsub("IESOLO","JESOLO").gsub("SCANZANO JONICO","SCANZANO").gsub("L AQUILA","L\' AQUILA").gsub("SCORZE","SCORZE\'").gsub("MONSUE","MONSUE\'")',

      'pol_residence_str' => 'res=profilefield[:pol_residence_str].gsub("SAN NICOLO","SAN NICOLO\'").gsub("SULL ARNO","SULL\' ARNO").gsub("FOSSO","FOSSO\'").gsub("D ALBA","D\'ALBA").gsub("L AQUILA","L\'AQUILA").gsub("CITTA ","CITTA\'").gsub("D EZZELINO","D\'EZZELINO").gsub("D ALME","D\'ALME\'").gsub("DE PASSERI","DE\' PASSERI").gsub("S CASCIANO","SAN CASCIANO").gsub("REGGIO NELL\'EMILIA","REGGIO EMILIA").gsub("SANT ELPIDIO","SANT\'ELPIDIO").gsub("DOLCE","DOLCE\'").gsub("SANT ANNA","SANT\'ANNA").gsub("ROVERE VERONESE","ROVERE\' VERONESE").gsub("D ARCANO","D\'ARCANO").gsub("CASTIGLIONE STIVIERE","CASTIGLIONE DELLE STIVIERE").gsub("MUGGIO","MUGGIO\'").gsub("D ASTI","D\'ASTI").gsub("SANT ELENA","SANT\'ELENA").gsub("D ESI","D\'ESI").gsub("D ADDA","D\'ADDA").gsub("DELL EMILIA","DELL\'EMILIA").gsub("MANSUE","MANSUE\'").gsub("FORLI","FORLI\'").gsub("SANT ANGELO","SANT\'ANGELO").gsub(" DELLE MOLE","").gsub("CITTA ","CITTA\' ").gsub("SULL ARNO","SULL\'ARNO").gsub("IESOLO","JESOLO").gsub("SCANZANO JONICO","SCANZANO").gsub("SCORZE","SCORZE\'")',
      'own_owner_specification_str' => 'copy_field',
      'own_owner_residence_province_str' =>  'translate_field'   ,
      'own_owner_residence_str' => 'res=profilefield[:own_owner_residence_str].gsub("SAN NICOLO","SAN NICOLO\'").gsub("FOSSO","FOSSO\'").gsub("D EZZELINO","D\'EZZELINO").gsub("D ALME","D\'ALME\'").gsub("DE PASSERI","DE\' PASSERI").gsub("S CASCIANO","SAN CASCIANO").gsub("REGGIO NELL\'EMILIA","REGGIO EMILIA").gsub("SANT ELPIDIO","SANT\'ELPIDIO").gsub("DOLCE","DOLCE\'").gsub("SANT ANNA","SANT\'ANNA").gsub("ROVERE VERONESE","ROVERE\' VERONESE").gsub("D ARCANO","D\'ARCANO").gsub("CASTIGLIONE STIVIERE","CASTIGLIONE DELLE STIVIERE").gsub("MUGGIO","MUGGIO\'").gsub("D ASTI","D\'ASTI").gsub("SANT ELENA","SANT\'ELENA").gsub("D ESI","D\'ESI").gsub("D ADDA","D\'ADDA").gsub("DELL EMILIA","DELL\'EMILIA").gsub("MANSUE","MANSUE\'").gsub("FORLI","FORLI\'").gsub("SANT ANGELO","SANT\'ANGELO").gsub(" DELLE MOLE","").gsub("CITTA ","CITTA\' ").gsub("SULL ARNO","SULL\'ARNO").gsub("IESOLO","JESOLO").gsub("SCANZANO JONICO","SCANZANO").gsub("SCORZE","SCORZE\'")',
      'driv_birth_state_str' => 'state=profilefield[:driv_birth_state_str].gsub("CINA","CINA REPUBBLICA POPOLARE").gsub("CECOSLOVACCHIA","REPUBBLICA CECA").gsub("PERU","PERU\'").gsub("IUGOSLAVIA","JUGOSLAVIA");',
