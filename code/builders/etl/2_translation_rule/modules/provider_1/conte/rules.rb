module Provider1Conte
  ...
  def build_hash_sect_1()
    ...
    @rule_values = {
      ...
      'veh_model_str' => 'make=profilefield[:veh_model_str]; make= make.gsub(/--->/,"" );make= make.gsub(/Â/,"" ) ; make= make.gsub(/à/,"a" ); make= make.gsub(/ì/,"i" );make= make.gsub(/ù/,"u" ); make= make.gsub(/°/,"a" );   regexpi_mod_array = make.split(" "); regexp_mod = "regexpi:([A-Za-z0-9])*";regexpi_mod_array.each do |el|; regexp_mod = regexp_mod + "(" + el.to_s + ".*)+(\\\\s)*" end;regexp_mod = regexp_mod + "$";',
