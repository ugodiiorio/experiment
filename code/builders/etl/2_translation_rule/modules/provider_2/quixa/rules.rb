module Provider2Quixa
  ...
  def build_hash_sect_1()
    ...
    @rule_values = {
      ...
      'pol_matriculation_date_str' => 'profilefield[:pol_matriculation_date_str].length > 2 ? (profilefield[:pol_matriculation_date_str].length == 10 ? profilefield[:pol_matriculation_date_str].slice(3,7) : copy_field )  : (x = "01/" + profilefield[:pol_matriculation_date_str].to_i.years).strftime("%Y")',
      ...
      'veh_set_up_str' => 'make=profilefield[:veh_set_up_str]; make= make.slice(0, make.length - 9); regexpi_mod_array = make.split(" "); regexp_mod = "regexpi:([A-Za-z0-9])*"; regexpi_mod_array.each do |el|; regexp_mod = regexp_mod + "(" + el.to_s + ".*)+(\\\\s)*" end; regexp_mod = regexp_mod + "$";',
