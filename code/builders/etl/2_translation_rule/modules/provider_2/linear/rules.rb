module Provider2Linear
  ...
  def build_hash_sect_1()
    ...
    @rule_values = {
      ...
      'veh_model_str' => 'make=profilefield[:veh_model_str]; make= make.gsub(/--->/,"" );make= make.gsub(/Â/,"" ) ; make= make.gsub(/V70\/XC70/,"V70" );  make= make.gsub(/\\([a-z0-9]+.*\\)/i, ""); make= make.gsub(/(serie)+(\\s)+([0-9][0-9])(^[0-9])/i,"\\\\3" ); make= make.gsub(/(serie)+(\\s)+([0-9][0-9][0-9])/i,"\\\\3" ); make= make.gsub(/(Vel Satis)/i,"Vel" ); make= make.gsub(/XF \\(X250\\)/,"XF" ); make= make.gsub(/mègane\/scènic/,"MEGANE" );make= make.gsub(/\\(/,"\\\\(" ); make= make.gsub(/\\)/,"\\\\)" ); marca=profilefield[:veh_make_str]; all=profilefield[:veh_set_up_str];  make= make.gsub(/à/,"a" ); make= make.gsub(/classe/,"" );make= make.gsub(/è/,"e" );make= make.gsub(/é/,"e" );make= make.gsub(/ì/,"i" );make= make.gsub(/---->/,"" );make= make.gsub(/--->/,"" );make= make.gsub(/ò/,"o" );make= make.gsub(/ù/,"u" ); make= make.gsub(/°/,"a" );   make= make.gsub(/(\\s)+[0-9]+ª(\\s)+s.*/i, "");  make= make.gsub(/\([0-9]+ª(\\s)+serie\)/i, "");make= make.gsub(/(\\s)+[0-9]+ª-*/i, ""); make= make.gsub(/\([0-9]+\)/i, ""); make= make.strip; regexpi_mod_array = make.split(" "); regexp_mod = "regexpi:^(" + make + ")+|regexpi:([A-Za-z0-9])*";regexpi_mod_array.each do |el|; regexp_mod = regexp_mod + "(" + el.to_s + ".*)+(\\\\s)*" end;regexp_mod = regexp_mod + "$";',
