module Provider2ZurichConnect
  ...
  def build_hash_sect_1()
    ...
    @rule_values = { 'pol_nr_of_paid_claims_3_yr_str'  => 'copy_field' ,
      ...
      'veh_set_up_str' => 'datas = (DateTime::now - profilefield[:pol_matriculation_date_str].to_i.year).strftime("%y"); make=profilefield[:veh_set_up_str]; make= make.gsub(/à/,"a" ); make= make.gsub(/è/,"é" );make= make.gsub(/ì/,"i" );make= make.gsub(/ò/,"o" );make= make.gsub(/ù/,"u" ); make= make.gsub(/°/,"a" ); make= make.gsub(/ª/,"a" ); regexpi_mod_array = make.split(" ");regexp_mod = "regexpi:([A-Za-z0-9])*";regexpi_mod_array.each do |el|;regexp_mod = regexp_mod + "(" + el.to_s + ".*)+(\\\\s)*" end;regexp_mod = regexp_mod + "([0-9]*/" + datas + ")*$";',
      ...
      'driv_job_str' => 'job=profilefield[:driv_job_str]; job= job.gsub(/pensionato/i,"pensionat" ); jregexp_mod = "regexpi:([A-Za-z0-9])*";jregexpi_mod_array = job.split("/"); jregexpi_mod_array.each do |el| ;el_array= el.split(" "); el_array.each do |el2|; el2.length < 3 ? nil : jregexp_mod = jregexp_mod + "(" + el2.to_s + ".*)|" end;end ;jregexp_mod = jregexp_mod + "(niente)\\\\b"', 'pol_matriculation_date_month_str' => 'profilefield[:pol_matriculation_date_str] == 10 ? copy_field : (Chronic.parse(@rate_date) - profilefield[:pol_matriculation_date_str].to_i.years).strftime("%-1m"); x = x.to_i - 1 ;y= "index=" + x.to_s',
      ...
      'pol_birth_date_month_str' => '(Chronic.parse(@rate_date) - profilefield[:pol_birth_date_str].to_i.years).strftime("%-1m"); x = x.to_i - 1 ;y= "index=" + x.to_s',
