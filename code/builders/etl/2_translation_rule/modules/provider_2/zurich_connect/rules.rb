module Provider2ZurichConnect
  ...
  def build_hash_sect_1()
    ...
    @rule_values = { 'pol_nr_of_paid_claims_3_yr_str'  => 'copy_field' ,
      ...
      'veh_set_up_str' =>  'copy_field' ,#'datas = (DateTime::now - profilefield[:pol_matriculation_date_str].to_i.year).strftime("%y"); make=profilefield[:veh_set_up_str]; make= make.gsub(/\\./,"\\\\." ); make= make.gsub(/à/,"a" ); make= make.gsub(/è/,"é" );make= make.gsub(/ì/,"i" );make= make.gsub(/ò/,"o" );make= make.gsub(/ù/,"u" ); make= make.gsub(/°/,"a" ); make= make.gsub(/ª/,"a" ); regexpi_mod_array = make.split(" ");regexp_mod = "regexpi:([A-Za-z0-9])*";regexpi_mod_array.each do |el|;regexp_mod = regexp_mod + "(" + el.to_s + ".*)+(\\\\s)*" end;regexp_mod = regexp_mod + "([0-9]*/" + datas + ")*$|regexpi:([A-Za-z0-9])*"; regexpi_mod_array = make.split(" "); regexpi_mod_array.each do |el|;el_array = el.scan(/\d+/);el_array.each do |num|; num = "(?=.*(" + num.to_s + ".*)+)(\\\\s)*"; regexp_mod = regexp_mod + num; end;el_array = el.scan(/\D+/); el_array.each do |char|;(char =~ /jt.*/i || char =~ /td.*/i || char =~ /cd.*/i || char =~ /sw|station|wagon/i )? char2 = "(?=.*(" + char.to_s + ".*)+)(\\\\s)*" : char2 = "(?=.*(" + char.to_s + ".*)*)(\\\\s)*";regexp_mod = regexp_mod + char2;end;end;regexp_mod = regexp_mod + "";',
