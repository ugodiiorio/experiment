module Provider2ZurichConnect
  ...
  def build_hash_sect_1()
    ...
    @rule_values = { 'pol_nr_of_paid_claims_3_yr_str'  => 'copy_field' ,
...
#      'veh_set_up_str' => '(Chronic.parse(@setup_date) - profilefield[:pol_matriculation_date_str].to_i.year).strftime("%y"); make=profilefield[:veh_set_up_str]; make= make.gsub(/\\./,"\\\\." ); make= make.gsub(/à/,"a" ); make= make.gsub(/è/,"é" );make= make.gsub(/ì/,"i" );make= make.gsub(/ò/,"o" );make= make.gsub(/ù/,"u" ); make= make.gsub(/°/,"a" ); make= make.gsub(/ª/,"a" ); make= make.gsub(/\\(/,"\\\\(" ); make= make.gsub(/\\)/,"\\\\)" );regexp_mod = "regexpi:([A-Za-z0-9])" ; regexpi_mod_array = make.split(" "); regexpi_mod_array.each do |el|; regexp_mod= regexp_mod + "*(" + el + ".*)+"; end ; regexp_mod = regexp_mod + "([0-9]*/" + datas + ")*(\\\\s)*|regexpi:([A-Za-z0-9])*"; regexpi_mod_array = make.split(" "); regexpi_mod_array.each do |el|; el =~ /([0-9]\\\\.[0-9])[a-z]{0,1}/  ? ( el = el.gsub(/([0-9]\\\\.[0-9])[a-z]{0,1}/, "\\\\1"); num = "(?=.*(" + el.to_s + ".*)+)(\\\\s)*" ; regexp_mod = regexp_mod + num;) : el_array = el.scan(/\d+/); el_array  ? (el_array.each do |num|; num = "(?=.*(" + num.to_s + ".*)+)(\\\\s)*"; regexp_mod = regexp_mod + num; end;el_array = el.scan(/\D+/); el_array.each do |char|;(char =~ /jt.*/i || char =~ /td.*/i || char =~ /cd.*/i || char =~ /sw|station|wagon/i )? char2 = "(?=.*(" + char.to_s + ".*)+)(\\\\s)*" : char2 = "(?=.*(" + char.to_s + ".*)*)(\\\\s)*";regexp_mod = regexp_mod + char2;end; ) : nil ;end;regexp_mod = regexp_mod + "";',
      'veh_set_up_str' => 'make=profilefield[:veh_set_up_str]; make= make.gsub(/\\./,"\\\\." ); make= make.gsub(/à/,"a" ); make= make.gsub(/è/,"é" );make= make.gsub(/ì/,"i" );make= make.gsub(/ò/,"o" );make= make.gsub(/ù/,"u" ); make= make.gsub(/°/,"a" ); make= make.gsub(/ª/,"a" ); make= make.gsub(/\\(/,"\\\\(" ); make= make.gsub(/\\)/,"\\\\)" );regexp_mod = "regexpi:([A-Za-z0-9])" ; regexpi_mod_array = make.split(" "); regexpi_mod_array.each do |el|; regexp_mod= regexp_mod + "*(" + el + ".*)+(\\\\s)*"; end ; regexp_mod = regexp_mod + "(\\\\s)*|regexpi:([A-Za-z0-9])*"; regexpi_mod_array = make.split(" "); regexpi_mod_array.each do |el|; el =~ /([0-9]\\\\.[0-9])[a-z]{0,1}/  ? ( el = el.gsub(/([0-9]\\\\.[0-9])[a-z]{0,1}/, "\\\\1"); num = "(?=.*(" + el.to_s + ".*)+)(\\\\s)*" ; regexp_mod = regexp_mod + num;) : el_array = el.scan(/\d+/); el_array  ? (el_array.each do |num|; num = "(?=.*(" + num.to_s + ".*)+)(\\\\s)*"; regexp_mod = regexp_mod + num; end;el_array = el.scan(/\D+/); el_array.each do |char|;(char =~ /jt.*/i || char =~ /td.*/i || char =~ /cd.*/i || char =~ /sw|station|wagon/i )? char2 = "(?=.*(" + char.to_s + ".*)+)(\\\\s)*" : char2 = "(?=.*(" + char.to_s + ".*)*)(\\\\s)*";regexp_mod = regexp_mod + char2;end; ) : nil ;end;regexp_mod = regexp_mod + "";',
...
#      'driv_job_str' => 'job=profilefield[:driv_job_str]; job= job.gsub(/pensionato/i,"pensionat" ); job= job.gsub(/disoccupato/i,"in cerca di occup." ); jregexp_mod = "regexpi:([A-Za-z0-9])*(";jregexpi_mod_array = job.split("/"); jregexpi_mod_array.each do |el| ;el_array= el.split(" "); el_array.each do |el2|; el2.length < 2 ? nil : jregexp_mod = jregexp_mod + "(" + el2.to_s + ".*)|" end;end ;jregexp_mod = jregexp_mod + "(niente))\\\\b"',
       'driv_job_str' => 'job=profilefield[:driv_job_str]; job= job.gsub(/pensionato/i,"pensionat" ); job= job.gsub(/disoccupato/i,"in cerca di occup." ); jregexp_mod = "regexpi:([A-Za-z0-9])*(" + job + ".*)$";',
      ...
      'pol_matriculation_date_month_str' => 'profilefield[:pol_matriculation_date_str].length > 2 ? (if profilefield[:pol_matriculation_date_str].length == 10 ; x= profilefield[:pol_matriculation_date_str].slice(3,2); x = x.to_i - 1 ;y= "index=" + x.to_s;   else  x= profilefield[:pol_matriculation_date_str].slice(0,2); x = x.to_i - 1 ;y= "index=" + x.to_s; end )  : x = "index= 0"',
  ...
  def build_hash_sect_2()
    ...
    @rule_values = {
      ...
      'pol_insurance_situation_str' => 'translate_field',
      'pol_matriculation_date_day_str' => 'profilefield[:pol_matriculation_date_str].length > 2 ? (profilefield[:pol_matriculation_date_str].length == 10 ? profilefield[:pol_matriculation_date_str].slice(3,7) : copy_field )  : x = (Chronic.parse(@rate_date) - profilefield[:pol_matriculation_date_str].to_i.years).strftime("%d")',
      'pol_matriculation_date_month_str' => 'profilefield[:pol_matriculation_date_str].length > 2 ? (if profilefield[:pol_matriculation_date_str].length == 10 ; x= profilefield[:pol_matriculation_date_str].slice(3,2); x = "index=" + x;   else  x= profilefield[:pol_matriculation_date_str].slice(0,2); x = "index=" + x ; end )  : x = "index= 1"',
      'pol_matriculation_date_str' => 'profilefield[:pol_matriculation_date_str].length > 2 ? ( if profilefield[:pol_matriculation_date_str].length == 10 ; x = profilefield[:pol_matriculation_date_str].slice(3,7) ; else  copy_field;  end )  : x = (Chronic.parse(@rate_date) - profilefield[:pol_matriculation_date_str].to_i.years).strftime("%m/%Y")',
      'pol_matriculation_date_year_num' => 'profilefield[:pol_matriculation_date_str].length > 2 ? ( if  profilefield[:pol_matriculation_date_str].length == 10 ;  x= profilefield[:pol_matriculation_date_str].slice(6,4) ; else  x = profilefield[:pol_matriculation_date_str].slice(3,4) end )  :  x = (Chronic.parse(@rate_date) - profilefield[:pol_matriculation_date_str].to_i.years).strftime("%Y")',
      'pol_nr_of_yrs_insured_in_the_last_5_yrs_str'  => 'translate_field',
      'pol_policy_starting_date_day_str'  => 'copy_field' ,
      ...
      'pol_subscriber_is_owner_str'  => 'translate_field',
      'veh_capacity_num'  => 'copy_field' ,
      'veh_make_str' =>  'copy_field' , #'make=profilefield[:veh_make_str];  regexpi_mod_array = make.split(" "); regexp_mod = "regexpi:([A-Za-z0-9])*"; regexpi_mod_array.each do |el|; regexp_mod = regexp_mod + "(" + el.to_s + ".*)+(\\\\s)*" end; regexp_mod = regexp_mod + "\\\\b";',
      'veh_model_str' =>  'copy_field' ,#'make=profilefield[:veh_model_str]; make= make.gsub(/ª s$/i, " s"); make= make.gsub(/ª/, " "); regexpi_mod_array = make.split(" "); regexp_mod = "regexpi:([A-Za-z0-9])*"; regexpi_mod_array.each do |el|; regexp_mod = regexp_mod + "(" + el.to_s + ".*)+(\\\\s)*" end; regexp_mod = regexp_mod + "\\\\b";',
      'veh_num_of_owners_str'  => 'translate_field' ,
      'veh_vehicle_value_str' => 'copy_field',
