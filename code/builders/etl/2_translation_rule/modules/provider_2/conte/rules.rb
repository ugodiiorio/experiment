module Provider2Conte
  ...
  def build_hash_sect_1()
    ...
    @rule_values = {
      ...
      'veh_new_used_vehicle_str'  => 'profilefield[:pol_matriculation_date_str].length == 10 ? (if Date.parse(profilefield[:pol_matriculation_date_str]).strftime("%Y") == (DateTime::now).strftime("%Y"); a="page:situazione_2:0"; else a="page:situazione_2:1" end ) : if profilefield[:pol_matriculation_date_str].to_i == 0  a="page:situazione_2:0"; else a="page:situazione_2:1" end )' ,
      ...
      'veh_make_str' => 'make=profilefield[:veh_make_str];  make= make.gsub(/à/,"a" ); make= make.gsub(/è/,"e" );make= make.gsub(/é/,"e" );make= make.gsub(/ì/,"i" );make= make.gsub(/ò/,"o" );make= make.gsub(/ù/,"u" ); make= make.gsub(/°/,"a" ); make= make.gsub(/ª/,"a" ); regexpi_mod_array = make.split(" "); regexp_mod = "regexpi:^";regexpi_mod_array.each do |el|; regexp_mod = regexp_mod + "(" + el.to_s + ".*)+(\\\\s)*" end;regexp_mod = regexp_mod + "\\\\b";',
       'veh_model_str' => 'make=profilefield[:veh_model_str]; make= make.gsub(/--->/,"" );make= make.gsub(/Â/,"" ) ; make= make.gsub(/à/,"a" ); make= make.gsub(/ì/,"i" );make= make.gsub(/ù/,"u" ); make= make.gsub(/°/,"a" );   regexpi_mod_array = make.split(" "); regexp_mod = "regexpi:([A-Za-z0-9])*(" + make + ")+|regexpi:([A-Za-z0-9])*";regexpi_mod_array.each do |el|; regexp_mod = regexp_mod + "(" + el.to_s + ")+(\\\\s)*" end;regexp_mod = regexp_mod + "$";',
     ...
     #      'driv_job_str' => 'job=profilefield[:driv_job_str]; jregexp_mod = "regexpi:([A-Za-z0-9])*(";jregexpi_mod_array = job.split("/"); jregexpi_mod_array.each do |el| ;el_array= el.split(" "); el_array.each do |el2|; el2.length < 3 ? nil : jregexp_mod = jregexp_mod + "(" + el2.to_s + ".*)|" end;end ;jregexp_mod = jregexp_mod + "(niente))\\\\b"',
      ...
      'veh_purchase_date_year_num' => 'profilefield[:veh_purchase_date_str] == 10 ? (profilefield[:pol_matriculation_date_str].length == 10 ? profilefield[:pol_matriculation_date_str].slice(3,7) : copy_field )  : (Chronic.parse(@rate_date) - profilefield[:veh_purchase_date_str].to_i.years).strftime("%Y")',
