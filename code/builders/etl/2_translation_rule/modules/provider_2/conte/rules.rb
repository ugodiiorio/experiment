...
#      'veh_new_used_vehicle_str'  => 'profilefield[:pol_matriculation_date_str].length == 10 ? (if Date.parse(profilefield[:pol_matriculation_date_str]).strftime("%Y") == (DateTime::now).strftime("%Y"); a="page:situazione_2:0"; else a="page:situazione_2:1" end ) : if profilefield[:pol_matriculation_date_str].to_i == 0  a="page:situazione_2:0"; else a="page:situazione_2:1" end )' ,
     ...
     'driv_job_str' => 'job=profilefield[:driv_job_str]; jregexp_mod = "regexpi:([A-Za-z0-9])*";jregexpi_mod_array = job.split("/"); jregexpi_mod_array.each do |el| ;el_array= el.split(" "); el_array.each do |el2|; el2.length < 3 ? nil : jregexp_mod = jregexp_mod + "(" + el2.to_s + ".*)|" end;end ;jregexp_mod = jregexp_mod + "(niente)\\\\b"',  'pol_matriculation_date_day_str' => 'profilefield[:pol_matriculation_date_str] == 10 ? copy_field : (Chronic.parse(@rate_date) - profilefield[:pol_matriculation_date_str].to_i.years).strftime("%d")',
      ...
      'veh_purchase_date_str' => 'profilefield[:veh_purchase_date_str] == 10 ? copy_field : (Chronic.parse(@rate_date) - profilefield[:veh_purchase_date_str].to_i.years).strftime("%d/%m/%Y")',
      'veh_purchase_date_year_num' => 'profilefield[:veh_purchase_date_str] == 10 ? copy_field : (Chronic.parse(@rate_date) - profilefield[:veh_purchase_date_str].to_i.years).strftime("%Y")',
      'veh_purchase_date_month_str' => 'profilefield[:veh_purchase_date_str] == 10 ? copy_field : (Chronic.parse(@rate_date) - profilefield[:veh_purchase_date_str].to_i.years).strftime("%m")',
      'veh_purchase_date_day_str' => 'profilefield[:veh_purchase_date_str] == 10 ? copy_field : (Chronic.parse(@rate_date) - profilefield[:veh_purchase_date_str].to_i.years).strftime("%d")',
