...
#      'veh_set_up_str' => 'make=profilefield[:veh_set_up_str]; regexpi_mod_array = make.split(" "); regexp_mod = "regexpi:([A-Za-z0-9])*"; regexpi_mod_array.each do |el|; regexp_mod = regexp_mod + "(" + el.to_s + ".*)+(\\\\s)*" end; regexp_mod = regexp_mod + "\\\\b";',
      'veh_set_up_str' => 'make=profilefield[:veh_set_up_str]; mod = profilefield[:veh_model_str]; make = make.gsub(make.scan(/#{mod}/i)[0].to_s, ""); make= make.gsub(/\\(/,"" ); make= make.gsub(/\\)/,"" ); make= make.gsub(/\\./,"\\\\." ); make= make.gsub(/(kw)+(\\\\s)*[0-9]{1,3}/,"" ); regexp_mod = "regexpi:([A-Za-z0-9])*" ; regexpi_mod_array = make.split(" "); regexpi_mod_array.each do |el|; regexp_mod= regexp_mod + "(" + el + ".*)+(\\\\s)*"; end ;regexp_mod = regexp_mod + "\\\\b";',
