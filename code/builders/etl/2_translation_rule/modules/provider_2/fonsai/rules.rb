module Provider2Fonsai
  ...
  def build_hash_sect_1()
    ...
    @rule_values = {
      ...
      'pol_driving_type_str' => 'if (profilefield[:pol_driving_type_str] == "esperta"); "Conducente esperto"; elsif (profilefield[:pol_driving_type_str] == "esclusiva"); if (profilefield[:pol_birth_date_str].to_i < 25); "Conducente esclusivo con età fino a 25 anni"; else; "Conducente esclusivo con età da 26 anni"; end; else; if (profilefield[:pol_client_type_str] == "persona fisica"); if (profilefield[:pol_driver_less_than_26_yrs_str] == "si"); "Qualunque conducente - no famigliari conviventi tra 18 e 25 anni"; else; "Qualunque conducente - famigliari conviventi tra 18 e 25 anni"; end; else; "Qualunque conducente - soggetti diversi dalle persone fisiche"; end; end',
      ...
      'veh_model_str' => 'make=profilefield[:veh_model_str];  regexpi_mod_array = make.split(" "); regexp_mod = "regexpi:^"; regexpi_mod_array.each do |el|; regexp_mod = regexp_mod + "(" + el.to_s + ".*)+(\\\\s)*" end; regexp_mod = regexp_mod + "\\\\b";',
...
#      'veh_set_up_str' => 'make=profilefield[:veh_set_up_str]; regexpi_mod_array = make.split(" "); regexp_mod = "regexpi:([A-Za-z0-9])*"; regexpi_mod_array.each do |el|; regexp_mod = regexp_mod + "(" + el.to_s + ".*)+(\\\\s)*" end; regexp_mod = regexp_mod + "\\\\b";',
      'veh_set_up_str' => 'make=profilefield[:veh_set_up_str]; make= make.gsub(/\\(/,"\\\\(" ); make= make.gsub(/\\)/,"\\\\)" ); make= make.gsub(/\\./,"\\\\." ); regexp_mod = "regexpi:([A-Za-z0-9])*" ; regexpi_mod_array = make.split(" "); regexpi_mod_array.each do |el|; regexp_mod= regexp_mod + "(" + el + ".*)+(\\\\s)*"; end ;regexp_mod = regexp_mod + "\\\\b";',
