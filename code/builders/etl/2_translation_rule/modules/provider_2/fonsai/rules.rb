module Provider2Fonsai
  ...
  def build_hash_sect_1()
    ...
    @rule_values = {
      ...
      'driv_job_str' => 'job=profilefield[:driv_job_str]; job= job.gsub("architetto","").gsub(/(commercilista|commercialista)/i,"libero professionista").gsub("dimostratore","").gsub("ecclesiastico","appartenente al clero").gsub("farmacista","").gsub("funzionario","").gsub("ingegnere","").gsub("magistrato","").gsub("militare","appartenente alle forze dell\'ordine").gsub("promotore",""); jregexp_mod = "regexpi:([A-Za-z0-9])*(";jregexpi_mod_array = job.split("/"); jregexpi_mod_array.each do |el| ;el_array= el.split(" "); el_array.each do |el2|; el2.length < 3 ? nil : jregexp_mod = jregexp_mod + "(" + el2.to_s + ".*)|" end;end ;jregexp_mod = jregexp_mod + "(niente))\\\\b"',
