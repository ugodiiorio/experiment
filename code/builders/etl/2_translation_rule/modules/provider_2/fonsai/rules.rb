module Provider2Fonsai
  ...
  def build_hash_sect_1()
    ...
    @rule_values = {
      ...
      'driv_job_str' => 'job=profilefield[:driv_job_str]; job= job.gsub("architetto","libero professionista").gsub(/(commercilista|commercialista)/i,"libero professionista").gsub("dimostratore","rappresentante").gsub("ecclesiastico","appartenente al clero").gsub("farmacista","libero professionista").gsub("funzionario","dirigente").gsub("ingegnere","libero professionista").gsub("magistrato","lavoratore dipendente pubblico").gsub("militare","appartenente alle forze dell\'ordine").gsub("promotore","rappresentante"); jregexp_mod = "regexpi:([A-Za-z0-9])*(";jregexpi_mod_array = job.split("/"); jregexpi_mod_array.each do |el| ;el_array= el.split(" "); el_array.each do |el2|; el2.length < 3 ? nil : jregexp_mod = jregexp_mod + "(" + el2.to_s + ".*)|" end;end ;jregexp_mod = jregexp_mod + "(niente))\\\\b"',
      ...
      'own_owner_zip_code_str' => 'profilefield[:own_owner_zip_code_str].gsub("24100","24121").gsub("25100","25121")',
      ...
      'pol_driving_type_str' => 'if (profilefield[:pol_driving_type_str] == "esperta"); "Conducente esperto"; elsif (profilefield[:pol_driving_type_str] == "esclusiva"); if (profilefield[:pol_birth_date_str].to_i < 25); "Conducente esclusivo con età fino a 25 anni"; else; "Conducente esclusivo con età da 26 anni"; end; else; if (profilefield[:pol_client_type_str] == "persona fisica"); if (profilefield[:pol_driver_less_than_26_yrs_str] == "no"); "Qualunque conducente - no famigliari conviventi tra 18 e 25 anni"; else; "Qualunque conducente - famigliari conviventi tra 18 e 25 anni"; end; else; "Qualunque conducente - soggetti diversi dalle persone fisiche"; end; end',
      ...
      'pol_partner_garages_str' => 'translate_field',
      ...
      'pol_residence_str' => 'res=profilefield[:pol_residence_str];  regexpi_mod_array = res.split(" "); regexp_mod = "regexpi:^"; regexpi_mod_array.each do |el|; regexp_mod = regexp_mod + "(" + el.to_s + ".*)+(\\\\s)*" end; regexp_mod = regexp_mod + "";',
      ...
      'veh_model_str' => 'make=profilefield[:veh_model_str]; marca=profilefield[:veh_make_str]; all=profilefield[:veh_set_up_str]; if ( marca =~ /mercedes/i && (make =~ /classe/i || make =~ /[a-z][0-9]{3}/i) ) ; make = make.gsub("classe", "") ; regexp_mod = "regexpi:^" + make.split(" ")[0].slice(0,1) + " " + make.split(" ")[0].slice(1,3) ; else regexpi_mod_array = make.split(" "); regexp_mod = "regexpi:^"; regexpi_mod_array.each do |el|; regexp_mod = regexp_mod + "(" + el.to_s + ".*)+(\\\\s)*" end; regexp_mod = regexp_mod + ""; end',
...
<<<<<<< HEAD:code/builders/etl/2_translation_rule/modules/provider_2/fonsai/rules.rb
