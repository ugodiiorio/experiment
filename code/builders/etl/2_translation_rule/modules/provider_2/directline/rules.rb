...
module Provider2Directline
  ...
  def build_hash_sect_1()
    ...
    @rule_values = {
      ...
      'veh_set_up_str' => 'model=profilefield[:veh_model_str];make=profilefield[:veh_set_up_str]; make= make.gsub(/à/,"a" ); make= make.gsub(/è/,"e" );make= make.gsub(/é/,"e" );make= make.gsub(/ì/,"i" );make= make.gsub(/ò/,"o" );make= make.gsub(/ù/,"u" ); make= make.gsub(/°/,"a" ); make= make.gsub(/ª/,"a" ); regexpi_mod_array = make.split(" "); regexp_mod = "regexpi:" + model + "([A-Za-z0-9])*";regexpi_mod_array.each do |el|; regexp_mod = regexp_mod + "(" + el.to_s + ".*)+(\\\\s)*" end;regexp_mod = regexp_mod + "$";',
  ...
  def build_hash_sect_2()
    ...
    @rule_values = {
      ...
      'pol_driver_less_than_26_yrs_str'  => 'translate_field' ,
      'pol_driver_accident_coverage_code_str' => 'translate_field' ,
      'pol_driver_accident_coverage_on_off_str' => 'copy_field' ,
      'pol_driver_accident_coverage_premium_id_str' => 'copy_field' ,
      ...
      'pol_matriculation_date_str' => 'european_date = \'%m/%Y\'; (Chronic.parse(@rate_date) - profilefield[:pol_matriculation_date_str].to_i.years)strftime(european_date).to_s' ,
