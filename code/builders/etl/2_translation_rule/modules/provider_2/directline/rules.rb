module Provider2Directline
  ...
  def build_hash_sect_1()
    ...
    @rule_values = {
      ...
      'driv_driving_license_yrs_str' => 'x = (profilefield[:driv_driving_license_yrs_str].to_i).to_i; if x == 1; y="Si, da 1 anno"; elsif x  < 3; y="Si, da 2 anni"; elsif x  == 3; y="Si, da 3 anni"; else  y="Si, da oltre 3 anni" end;',
      'driv_heir_str'  => 'copy_field' ,
      'driv_italian_residence_starting_yrs_num'  => 'copy_field' ,
      'driv_job_2_str'  => 'copy_field' ,
      'driv_job_str' => 'job=profilefield[:driv_job_str]; spec=profilefield[:own_owner_specification_str] == "C" ? job = "impiegato" : job=profilefield[:driv_job_str]; job=job.gsub("militare", "forze armate"); job=job.gsub("disoccupato", "senza occupazione"); jregexp_mod = "regexpi:([A-Za-z0-9])*(";jregexpi_mod_array = job.split("/"); jregexpi_mod_array.each do |el| ;el_array= el.split(" "); el_array.each do |el2|; el2.length < 3 ? nil : jregexp_mod = jregexp_mod + "(" + el2.to_s + ".*)|" end;end ;jregexp_mod = jregexp_mod + "(niente))\\\\b"',
      ...
      'pol_already_benefit_from_bersani_str'  => 'translate_field' ,
      ...
      'pol_matriculation_date_str' => 'profilefield[:pol_matriculation_date_str].length > 2 ? (profilefield[:pol_matriculation_date_str].length == 10 ? profilefield[:pol_matriculation_date_str].slice(3,7) : copy_field )  :  ( if profilefield[:pol_matriculation_date_str].to_i == 0 ; x = (DateTime::now).strftime("%m/%Y"); else x= "01/" + (Chronic.parse(@rate_date) - profilefield[:pol_matriculation_date_str].to_i.years).strftime("%Y"); end );',
      ...
      'pol_other_policies_with_company_str'  => 'translate_field' ,
      ...
      'veh_model_str' => 'make=profilefield[:veh_model_str];make= make.gsub(/---->/,"-----" );make= make.gsub(/--->/,"----" ); make= make.gsub(/à/,"a" ); make= make.gsub(/è/,"e" );make= make.gsub(/é/,"e" );make= make.gsub(/ì/,"i" );make= make.gsub(/ò/,"o" );make= make.gsub(/ù/,"u" ); make= make.gsub(/°/,"a" );  make= make.gsub(/ª/i, "a"); make= make.gsub(/ª/, "a ");  make= make.gsub(/\\(/,"\\\\(" ); make= make.gsub(/\\)/,"\\\\)" ); regexpi_mod_array = make.split(" "); regexp_mod = "regexpi:^(" + make + ")+|regexpi:^";regexpi_mod_array.each do |el|; regexp_mod = regexp_mod + "(" + el.to_s + ".*)+(\\\\s)*" end;regexp_mod = regexp_mod + "$";',
      #      'veh_set_up_str' => 'model=profilefield[:veh_model_str];make=profilefield[:veh_set_up_str]; make= make.gsub(/à/,"a" ); make= make.gsub(/è/,"e" );make= make.gsub(/é/,"e" );make= make.gsub(/ì/,"i" );make= make.gsub(/ò/,"o" );make= make.gsub(/ù/,"u" ); make= make.gsub(/°/,"a" ); make= make.gsub(/ª/,"a" ); regexpi_mod_array = make.split(" "); regexp_mod = "regexpi:" + model + "(\\\\s)*([A-Za-z0-9])*";regexpi_mod_array.each do |el|; regexp_mod = regexp_mod + "(" + el.to_s + ".*)+(\\\\s)*" end;regexp_mod = regexp_mod + "$";',
      #      'veh_set_up_str' => 'make=profilefield[:veh_set_up_str]; make= make.gsub(/\\./,"\\\\." ); make= make.gsub(/à/,"a" ); make= make.gsub(/è/,"e" );make= make.gsub(/é/,"e" );make= make.gsub(/ì/,"i" );make= make.gsub(/ò/,"o" );make= make.gsub(/ù/,"u" ); make= make.gsub(/°/,"a" ); make= make.gsub(/ª/,"a" ); regexpi_mod_array = make.split(" "); regexp_mod = "regexpi:([A-Za-z0-9])*(" + make + ")+|regexpi:([A-Za-z0-9])*";regexpi_mod_array.each do |el|; regexp_mod = regexp_mod + "(" + el.to_s + ".*)+(\\\\s)*" end;regexp_mod = regexp_mod + "$";',
      'veh_set_up_str' => 'make=profilefield[:veh_set_up_str]; make= make.gsub(/\\./,"\\\\." );  make= make.gsub(/à/,"a" ); make= make.gsub(/è/,"e" );make= make.gsub(/é/,"e" );make= make.gsub(/ì/,"i" );make= make.gsub(/ò/,"o" );make= make.gsub(/ù/,"u" ); make= make.gsub(/°/,"a" ); make= make.gsub(/ª/,"a" ); make= make.gsub(/\\(/,"\\\\(" ); make= make.gsub(/\\)/,"\\\\)" );regexpi_mod_array = make.split(" "); regexp_mod = "regexpi:([A-Za-z0-9])*(" + make + ")+|regexpi:([A-Za-z0-9])*"; regexpi_mod_array = make.split(" "); regexpi_mod_array.each do |el|;el_array = el.scan(/\d+/);el_array.each do |num|; num = "(?=.*(" + num.to_s + ".*)+)(\\\\s)*"; regexp_mod = regexp_mod + num; end;el_array = el.scan(/\D+/); el_array.each do |char|;(char =~ /jt.*/i || char =~ /td.*/i || char =~ /cd.*/i || char =~ /sw|station|wagon/i )? char2 = "(?=.*(" + char.to_s + ".*)+)(\\\\s)*" : char2 = "(?=.*(" + char.to_s + ".*)*)(\\\\s)*";regexp_mod = regexp_mod + char2;end;end;regexp_mod = regexp_mod + "";',
      ...
      #
      'veh_purchase_date_day_str' => 'profilefield[:veh_purchase_date_str].length > 2 ? (profilefield[:veh_purchase_date_str].length == 10 ? profilefield[:veh_purchase_date_str].slice(3,7) : copy_field )  : (Chronic.parse(@rate_date) - profilefield[:veh_purchase_date_str].to_i.years).strftime("%d")',
      'veh_purchase_date_month_str' => 'profilefield[:veh_purchase_date_str].length > 2 ? (profilefield[:veh_purchase_date_str].length == 10 ? profilefield[:veh_purchase_date_str].slice(3,7) : copy_field )  : (Chronic.parse(@rate_date) - profilefield[:veh_purchase_date_str].to_i.years).strftime("%m")',
      'veh_purchase_date_str' => 'profilefield[:veh_purchase_date_str].length > 2 ? (profilefield[:veh_purchase_date_str].length == 10 ? profilefield[:veh_purchase_date_str].slice(3,7) : copy_field )  : (Chronic.parse(@rate_date) - profilefield[:veh_purchase_date_str].to_i.years).strftime("%m/%Y")',
      'veh_purchase_date_year_num' => 'profilefield[:veh_purchase_date_str].length > 2 ? (profilefield[:veh_purchase_date_str].length == 10 ? profilefield[:veh_purchase_date_str].slice(6,4) : copy_field )  : (Chronic.parse(@rate_date) - profilefield[:veh_purchase_date_str].to_i.years).strftime("%Y")',
  ...
  def build_hash_sect_2()
    ...
    @rule_values = {
      ...
      #paolo pol_bersani_policy_expiring_date_str' => 'Chronic.parse(profilefield[:pol_bersani_policy_expiring_date_str]).strftime("%d/%m/%Y");',
      'pol_bersani_ref_vehicle_insured_with_company_str'  => 'translate_field' ,
      'pol_bersani_ref_vehicle_number_plate_str'  => 'copy_field' ,
      'pol_bersani_str'  => 'translate_field' ,
      ...
      'pol_number_of_NI_NA_yrs_during_5_yrs_str'  => 'copy_field' ,
      'pol_number_plate_type_str'  => 'copy_field' ,
# paolo     'pol_other_policies_with_company_str'  => 'translate_field' ,
      'pol_other_vehicle_use_str'  => 'copy_field' ,
      'pol_partner_garages_str'  => 'copy_field' ,
      'pol_passenger_str'  => 'copy_field' ,
      'pol_payment_str'  => 'copy_field' ,
      ...
      'veh_loading_unloading_str'  => 'copy_field' ,
      'veh_make_previous_vehicle_str'  => 'copy_field' ,
      'veh_make_str' => 'make=profilefield[:veh_make_str];  regexpi_mod_array = make.split(" "); regexp_mod = "regexpi:^(" + make + ")$";',
...
#
      ...
      'veh_purchase_date_month_str' => 'profilefield[:veh_purchase_date_str].length > 2 ? (profilefield[:veh_purchase_date_str].length == 10 ? profilefield[:veh_purchase_date_str].slice(3,7) : copy_field )  : (Chronic.parse(@rate_date) - profilefield[:veh_purchase_date_str].to_i.years).strftime("%m")',
      'veh_purchase_date_str' => 'profilefield[:veh_purchase_date_str].length > 2 ? (profilefield[:veh_purchase_date_str].length == 10 ? profilefield[:veh_purchase_date_str].slice(3,7) : copy_field )  : (Chronic.parse(@rate_date) - profilefield[:veh_purchase_date_str].to_i.years).strftime("%m/%Y")',
      'veh_purchase_date_year_num' => 'profilefield[:veh_purchase_date_str].length > 2 ? (profilefield[:veh_purchase_date_str].length == 10 ? profilefield[:veh_purchase_date_str].slice(3,7) : copy_field )  : (Chronic.parse(@rate_date) - profilefield[:veh_purchase_date_str].to_i.years).strftime("%Y")',
...
#      'pol_nr_of_paid_claims_4_yr_str'  => 'a= profilefield[:pol_nr_of_paid_claims_this_yr_str].to_i; b= profilefield[:pol_nr_of_paid_claims_1_yr_str].to_i; c= profilefield[:pol_nr_of_paid_claims_2_yr_str].to_i; d= profilefield[:pol_nr_of_paid_claims_3_yr_str].to_i; e= profilefield[:pol_nr_of_paid_claims_4_yr_str].to_i; sin= a+b+c+d+e; (sin > 0) ? "/html/body/table[2]/tbody/tr/td[4]/form/table[4]/tbody/tr[8]/td[4]/table/tbody/tr/td/input" : "/html/body/table[2]/tbody/tr/td[4]/form/table[4]/tbody/tr[8]/td[4]/table/tbody/tr/td[3]/input"',
...
#      'veh_purchase_date_str' => '(Chronic.parse(@rate_date) - profilefield[:pol_matriculation_date_str].to_i.years).strftime("%d%m%Y")' ,
