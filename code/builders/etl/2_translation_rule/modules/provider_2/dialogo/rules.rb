module Provider2Dialogo
  ...
  def build_hash_sect_1()
    ...
    @rule_values = {
      ...
      'veh_model_str' => 'make=profilefield[:veh_model_str]; make= make.gsub(/à/,"a" ); make= make.gsub(/è/,"e" );make= make.gsub(/é/,"e" );make= make.gsub(/ì/,"i" );make= make.gsub(/ò/,"o" );make= make.gsub(/ù/,"u" ); make= make.gsub(/°/,"a" ); make= make.gsub(/[0-9]+ª(\\s)+s.*/i, "");  make= make.gsub(/ª/,"a" );regexpi_mod_array = make.split("/"); regexpi_mod_array = regexpi_mod_array[0].split(" "); regexpi_mod_array = make.split(" "); regexp_mod = "regexpi:^";regexpi_mod_array.each do |el|; regexp_mod = regexp_mod + "(" + el.to_s + ".*)+(\\\\s)*" end;regexp_mod = regexp_mod + "\\\\b";',
