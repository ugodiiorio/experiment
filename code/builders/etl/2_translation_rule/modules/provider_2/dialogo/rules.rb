module Provider2Dialogo
  ...
  def build_hash_sect_1()
    ...
    @rule_values = {
     ...
     'veh_make_str' => 'make=profilefield[:veh_make_str]; make= make.gsub(/---->/,"" ); make= make.gsub(/--->/,"" );make= make.gsub(/Â/,"" ); make= make.gsub(/à/,"a" ); make= make.gsub(/è/,"e" );make= make.gsub(/é/,"e" );make= make.gsub(/ì/,"i" );make= make.gsub(/ò/,"o" );make= make.gsub(/ù/,"u" ); make= make.gsub(/°/,"a" ); make= make.gsub(/ª/,"a" ); regexpi_mod_array = make.split(" "); regexp_mod = "regexpi:^(" + make + ")+|regexpi:([A-Za-z0-9])*";regexpi_mod_array.each do |el|; regexp_mod = regexp_mod + "(" + el.to_s + ")+(\\\\s)*" end;regexp_mod = regexp_mod + "$";',
      'veh_model_str' => 'make=profilefield[:veh_model_str]; make= make.gsub(/--->/,"" );make= make.gsub(/Â/,"" ) ; make= make.gsub(/\\(/,"\\\\(" ); make= make.gsub(/\\)/,"\\\\)" ); marca=profilefield[:veh_make_str]; all=profilefield[:veh_set_up_str]; if  marca == "BMW" ; regexp_mod = "regexpi:^" + all.split(" ")[0] else make= make.gsub(/à/,"a" ); make= make.gsub(/è/,"e" ); make= make.gsub(/V70\/XC70/,"XC70" ); make= make.gsub(/é/,"e" ); make= make.gsub(/ì/,"i" ); make= make.gsub(/ò/,"o" ); make= make.gsub(/ù/,"u" );make= make.gsub(/°/,"a" ); make= make.gsub(/[0-9]+ª(\\s)+s.*/i, "");make= make.gsub(/\\([0-9]+.*\\)/i, "");make= make.gsub(/ª/,"a" ); regexp_mod = "regexpi:^"; regexpi_mod_array = make.split("/"); regexpi_mod_array.each do |el|;regexpi_mod_array2 = el.split(" ");regexpi_mod_array2.each do |el2|;regexp_mod = regexp_mod + "(" + el2.to_s + ".*)+(\\\\s)*" end; end; regexp_mod = regexp_mod + "\\\\b" end;',
