module Provider2Dialogo
  ...
  def build_hash_sect_1()
    ...
    @rule_values = {
      ...
      'driv_job_str' => 'job=profilefield[:driv_job_str]; spec=profilefield[:own_owner_specification_str] == "C" ? "impiegato" : job=job.gsub("imprenditore / industriale", "libero professionista") ; jregexp_mod = "regexpi:([A-Za-z0-9])*(";jregexpi_mod_array = job.split("/"); jregexpi_mod_array.each do |el| ;el_array= el.split(" "); el_array.each do |el2|; el2.length < 3 ? nil : jregexp_mod = jregexp_mod + "(" + el2.to_s + ".*)|" end;end ;jregexp_mod = jregexp_mod + "(niente))\\\\b"','driv_residence_same_as_home_address_str' => 'copy_field',
      ...
      'pol_nr_of_paid_claims_3_yr_str' => 'x= profilefield[:pol_nr_of_paid_claims_this_yr_str]; y= profilefield[:pol_nr_of_paid_claims_1_yr_str]; z= profilefield[:pol_nr_of_paid_claims_2_yr_str]; t= profilefield[:pol_nr_of_paid_claims_3_yr_str];   sin= x.to_i+y.to_i+z.to_i+t.to_i;',
      ...
      'veh_make_str' => 'make=profilefield[:veh_make_str]; make= make.gsub(/---->/,"" ); make= make.gsub(/--->/,"" );make= make.gsub(/(landrover)/i,"land rover" );make= make.gsub(/Â/,"" ); make= make.gsub(/à/,"a" ); make= make.gsub(/è/,"e" );make= make.gsub(/é/,"e" );make= make.gsub(/ì/,"i" );make= make.gsub(/ò/,"o" );make= make.gsub(/ù/,"u" ); make= make.gsub(/°/,"a" ); make= make.gsub(/ª/,"a" );make= make.gsub(/(landrover)/i,"land rover" ); regexpi_mod_array = make.split(" "); regexp_mod = "regexpi:^(" + make + ")+"',
      'veh_marble_blocks_str' => 'copy_field',
      'veh_model_str' => 'make=profilefield[:veh_model_str]; make= make.gsub(/\\([0-9]{4}--->\\)/,"" );make= make.gsub(/Polo 1ª-2ª\/Derby/i,"POLO" );make= make.gsub(/mégane\\/scénic/i,"MEGANE" );make= make.gsub(/punto evo/i,"punto" ); make= make.gsub(/Tigra TwinTop/i,"tigra" ); make= make.gsub(/Focus 2\\/Focus C-Max/i,"focus" ); make= make.gsub(/Combo\\/corsa.*/i,"combo" ); make= make.gsub(/\'96/,"" );make= make.gsub(/\\((\\\\s)*\'[0-9]{2}(\\\\s)*-{0,1}(\\\\s)*(\')*[0-9]{0,2}(\\\\s)*\\)/,"" );make= make.gsub(/Voy.\/G.Voyager/i, "grand voyager" );make= make.gsub(/(grand vitara)(\\\\s)*(.*)/i, "\\\\1" );make= make.gsub(/Â/,"" ) ;  make= make.gsub(/\\(/,"\\\\(" ); make= make.gsub(/\\)/,"\\\\)" ); make= make.gsub(/(land cruiser 100)/i,"land cruiser" ); make= make.gsub(/megan\/scenic/i,"megane" ); make= make.gsub(/(rexton\\/rexton II)/i,"rexton" ); make= make.gsub(/à/,"a" ); make= make.gsub(/è/,"e" ); make= make.gsub(/V70\/XC70/,"XC70" ); make= make.gsub(/é/,"e" ); make= make.gsub(/ì/,"i" ); make= make.gsub(/ò/,"o" ); make= make.gsub(/ù/,"u" );make= make.gsub(/°/,"a" ); make= make.gsub(/[0-9]+ª(\\s)+s.*/i, "");make= make.gsub(/[0-9]+ª(\\s)+serie/i, "");make= make.gsub(/\\([0-9]+.*\\)/i, "");make= make.gsub(/ª/,"a" );marca=profilefield[:veh_make_str]; all=profilefield[:veh_set_up_str]; all= all.gsub(all.scan(/#{make}/i)[0].to_s, ""); if  marca =~ /BMW/i ; regexp_mod = "regexpi:^" + all.split(" ")[1]; elsif (marca == "ROVER" && make =~ /serie/i); regexp_mod = "regexpi:^" + all.split(" ")[0] ;   elsif (marca =~ /volvo/i && make =~ /serie.*/i) ; regexp_mod = "regexpi:^" + all.split(" ")[0] ;elsif (marca =~ /fiat/i && make =~ /bravo\/brava/i) ; regexp_mod = "regexpi:^" + all.split(" ")[0] ; elsif (marca == "MERCEDES" && make =~ /classe|serie/i) ; regexp_mod = "regexpi:^(" + all.split(" ")[0] + " " + all.split(" ")[1]+")|regexpi:^(" + all.split(" ")[0] + ")"; elsif (marca == "NISSAN" && make =~ /^z$/i) ; regexp_mod = "regexpi:^" + all.split(" ")[0]  ; else  regexp_mod = "regexpi:^"; regexpi_mod_array = make.split("/"); regexpi_mod_array.each do |el|;regexpi_mod_array2 = el.split(" ");regexpi_mod_array2.each do |el2|;regexp_mod = regexp_mod + "(" + el2.to_s + ".*)+(\\\\s)*" end; end; regexp_mod = regexp_mod + "\\\\b" end;',

#      'veh_model_str' => 'make=profilefield[:veh_model_str]; make= make.gsub(/\\([0-9]{4}--->\\)/,"" );make= make.gsub(/Â/,"" ) ; make= make.gsub(/\\(/,"\\\\(" ); make= make.gsub(/\\)/,"\\\\)" ); make= make.gsub(/(land cruiser 100)/i,"land cruiser" ); make= make.gsub(/mondeo \'96/i,"mondeo" );make= make.gsub(/megane\\/scenic/i,"megane" ); make= make.gsub(/(rexton\\/rexton II)/i,"rexton" ); make= make.gsub(/à/,"a" ); make= make.gsub(/è/,"e" ); make= make.gsub(/V70\/XC70/,"XC70" ); make= make.gsub(/é/,"e" ); make= make.gsub(/ì/,"i" ); make= make.gsub(/ò/,"o" ); make= make.gsub(/ù/,"u" );make= make.gsub(/°/,"a" ); make= make.gsub(/[0-9]+ª(\\s)+s.*/i, "");make= make.gsub(/\\([0-9]+.*\\)/i, "");make= make.gsub(/ª/,"a" ); regexp_mod = "regexpi:^" + make + "$|regexpi:^"; regexpi_mod_array = make.split("/"); regexpi_mod_array.each do |el|;regexpi_mod_array2 = el.split(" ");regexpi_mod_array2.each do |el2|;regexp_mod = regexp_mod + "(" + el2.to_s + ".*)+(\\\\s)*" end; end; regexp_mod = regexp_mod + "$";',
...
#      'veh_set_up_str' => 'kw=profilefield[:veh_kw_num].to_s;make=profilefield[:veh_set_up_str]; make= make.gsub(/porte/,"PT" ); make= make.gsub(/turbodiesel/,"TD" );make= make.gsub(/weekend/,"WE" ); make= make.gsub(/raisin-top/,"R-TP" ); make= make.gsub(/twin spark/,"T-SK" );make= make.gsub(/\\./,"\\\\." ); make= make.gsub(/à/,"a" ); make= make.gsub(/è/,"e" );make= make.gsub(/é/,"e" );make= make.gsub(/ì/,"i" );make= make.gsub(/ò/,"o" );make= make.gsub(/ù/,"u" ); make= make.gsub(/°/,"a" ); make= make.gsub(/ª/,"a" ); make= make.gsub(/tddi/i, "TD DI");  make= make.gsub(/tdci/i, "TD CI"); make= make.gsub(/5p./i, "5 PT");make= make.gsub(/3p./i, "3 PT");  make= make.gsub(/turbodiesel/i, "TD"); make= make.gsub(/\\(/,"\\\\(" ); make= make.gsub(/\\)/,"\\\\)" ); regexpi_mod_array = make.split(" ");regexp_mod = "(([A-Za-z0-9])*";regexpi_mod_array.each do |el|;el_array = el.scan(/\d+/);el_array.each do |num|;num = "(?=.*(" + num.to_s + ".*)+)(\\\\s)*";regexp_mod = regexp_mod + num;end;el_array = el.scan(/\D+/);el_array.each do |char|;(char =~ /jt.*/i || char =~ /td.*/i || char =~ /cd.*/i || char =~ /sw|station|wagon/i )? char2 = "(?=.*(" + char.to_s + ".*)+)(\\\\s)*" : char2 = "(?=.*(" + char.to_s + ".*)+)(\\\\s)*";regexp_mod = regexp_mod + char2;end;end;regexp_mod = regexp_mod + ").+(([A-Za-z0-9])*" + kw + ".* kw)|";regexp_mod = regexp_mod +"(([A-Za-z0-9])*(\\\\s)*(?=.*?\\\\b(" + regexpi_mod_array[0].to_s + ")*\\\\b)*" ;index=0 ;for index in (1..regexpi_mod_array.length-1);regexp_mod = regexp_mod + "((?=.*?\\\\b" + regexpi_mod_array[index].to_s + "\\\\b))*" end;regexp_mod = regexp_mod + ").+(([A-Za-z0-9])*" + kw + ".* kw)";',
      'veh_set_up_str' => 'kw=profilefield[:veh_kw_num].to_s;make=profilefield[:veh_set_up_str]; mod = profilefield[:veh_model_str]; make = make.gsub(make.scan(/#{mod}/i)[0].to_s, ""); make= make.gsub(/[0-9]+a(\\s)+serie/i, "");make= make.gsub(/porte/,"PT" ); make= make.gsub(/turbodiesel/,"TD" );make= make.gsub(/weekend/,"WE" ); make= make.gsub(/raisin-top/,"R-TP" ); make= make.gsub(/twin spark/,"T-SK" ); make= make.gsub(/à/,"a" ); make= make.gsub(/è/,"e" );make= make.gsub(/é/,"e" );make= make.gsub(/ì/,"i" );make= make.gsub(/ò/,"o" );make= make.gsub(/ù/,"u" ); make= make.gsub(/°/,"a" ); make= make.gsub(/ª/,"a" ); make= make.gsub(/tddi/i, "TD DI");  make= make.gsub(/tdci/i, "TD CI"); make= make.gsub(/5p./i, "5 PT");make= make.gsub(/3p./i, "3 PT");  make= make.gsub(/turbodiesel/i, "TD"); make= make.gsub(/e36/i,"" ); make= make.gsub(/e46/i,"" ); make= make.gsub(/\\(/,"" ); make= make.gsub(/\\)/,"" ); make= make.gsub(/\\./,"\\\\." );regexp_mod = "regexpi:([A-Za-z0-9])*("; regexpi_mod_array = make.split(" "); make.scan(/([0-9])+(\s)*(porte|pt|p(\.)*)+/i)[0].to_s == "" ? porte = "" : porte = "(?=.*((" + make.scan(/([0-9])+(\\s)*(porte|pt|p(\\.)*)+/i)[0].to_s.gsub(/([0-9])+((\\s)*(porte|pt|p(\\.)*))+/i, "\\\\1") + ")+(\\\\s)*pt)+)(\\\\s)*"; make2 = make.gsub(/([0-9])+(\\s)*(porte|pt|p(\\.)*)+/i, "");regexpi_mod_array = make2.split(" ");regexpi_mod_array.each do |el|;if el =~ /([0-9]\\\\.[0-9])[a-z]{0,1}/i;( el = el.gsub(/([0-9]\\\\.[0-9])[a-z]{0,1}/, "\\\\1"); num = "(?=.*(" + el.to_s + ".*)+)(\\\\s)*" ; regexp_mod = regexp_mod + num;); elsif el =~ /([0-9]{1,3})(\\\\s)*(cv|v)/i; num = "(?=.*(" + el.to_s + ".*)+)(\\\\s)*" ; regexp_mod = regexp_mod + num;  elsif el_array = el.scan(/\d+/); el_array  ? (el_array.each do |num|; num = "(?=.*(" + num.to_s + ".*)+)(\\\\s)*";regexp_mod = regexp_mod + num;end;el_array = el.scan(/\D+/);el_array.each do |char|;(char =~ /jt.*/i || char =~ /td.*/i || char =~ /cd.*/i || char =~ /sw|station|wagon|(s\\.w\\.)/i )?char2 = "(?=.*(" + char.to_s + ".*)+)(\\\\s)*" : char2 = "(?=.*(" + char.to_s + ".*)*)(\\\\s)*";regexp_mod = regexp_mod + char2;end; ) : nil ;end; end;regexp_mod = regexp_mod + porte + ").+(([A-Za-z0-9])*(\\\\s)*(" + kw + " kw))|";regexpi_mod_array = make.split(" ");regexp_mod = regexp_mod +"regexpi:(([A-Za-z0-9])*(\\\\s)*(?=.*?\\\\b(" + regexpi_mod_array[0].to_s + ")*\\\\b)*" ;index=0 ;for index in (1..regexpi_mod_array.length-1);regexp_mod = regexp_mod + "((?=.*?\\\\b" + regexpi_mod_array[index].to_s + "\\\\b))*" end;regexp_mod = regexp_mod + ").+(([A-Za-z0-9])*(\\\\s)*(" + kw + " kw))";',
#
